...
function ENT:Use( activator, caller )
	...
	if(!cf.InstantSellMode) then
		...
		if(self.cigsStored>0) then
			local multiplier = 1 -- Varsayılan çarpan
			if CF_GetPlayerSellMultiplier then -- Fonksiyonun var olup olmadığını kontrol et
				multiplier = CF_GetPlayerSellMultiplier(caller)
			else
				-- Fonksiyon yoksa konsola bir uyarı yazdır ve normal devam et
				print("[Cigarette Factory] UYARI: CF_GetPlayerSellMultiplier fonksiyonu bulunamadı. Rütbe çarpanları uygulanmayacak.")
			end
			
			local finalPrice = math.floor(self.cigsStored * cf.sellPrice * multiplier)
			caller:addMoney(finalPrice)
			SendNotification(caller, 0, 4, cf.SellText1..self.cigsStored..cf.SellText2..finalPrice..cf.CurrencyText.."!")
