...
-- Makine sahibini ayarla (spawn hook'unda kullanılmalı)
...
function ENT:Use( activator, caller )
	...
	-- Makine sahibini belirle (ilk kullanan kişi)
	if not IsValid(self.ownerPlayer) and IsValid(caller) and caller:IsPlayer() then
		self.ownerPlayer = caller
	end
	...
	if(self.switchedOn) then 
		...
		-- Makine sahibinin paket sayısını kontrol et
		local packCount = CF_GetPlayerPackCount(self.ownerPlayer)
		if packCount >= 3 then
			if IsValid(caller) and caller:IsPlayer() then
				caller:ChatPrint("Sahip olduğunuz sigara paketi sayısı: " .. packCount .. "/3 - Önce mevcut paketleri satın veya depolayın!")
			end
			self:EmitSound("buttons/button10.wav", 80, 100, 1)
			return
		end
...
function ENT:Think()
	if(self.shouldOccur) then
		...
		if IsValid(self.ownerPlayer) then
			local packCount = CF_GetPlayerPackCount(self.ownerPlayer)
			if packCount >= 3 and self.switchedOn then
				self.switchedOn = false
				self.productionTime = 0
				self.LoopSound:Stop()
				self.ownerPlayer:ChatPrint("Makine durdu: Maksimum sigara paketi limitine (3) ulaştınız!")
			end
		end
		...
		if(self.productionTime>=math.ceil(cf.timeToProduce/(self.engineUpgrade and cf.engineUpgradeBoost or 1))) then
			-- Üretmeden önce tekrar kontrol et
			if IsValid(self.ownerPlayer) then
				local packCount = CF_GetPlayerPackCount(self.ownerPlayer)
				if packCount >= 3 then
					self.switchedOn = false
					self.productionTime = 0
					self.LoopSound:Stop()
					self.ownerPlayer:ChatPrint("Üretim durdu: Maksimum paket limitine ulaştınız!")
				else
					...
					local CigPack = ents.Create("cf_cigarette_pack")
					CigPack:SetPos(fixedPos)
					CigPack:SetAngles(fixedAng)
					CigPack:SetVelocity(fixedPos)
					CigPack:Spawn()
					
					-- Paketi oyuncuya kaydet
					CF_AddPackToPlayer(self.ownerPlayer, CigPack)
					
					-- Paketin sahibini ayarla (paket entity'sinde kullanmak için)
					CigPack.ownerPlayer = self.ownerPlayer
					
					self:EmitSound(self.EjectSound, 60, 200, 1)
				end
			else
				-- Sahip yoksa üretimi durdur
				self.switchedOn = false
				self.productionTime = 0
				self.LoopSound:Stop()
			end
