		...
		if IsValid(caller) and caller:IsPlayer()
			...
			if swep and IsValid(swep) then
				...
				if ammoType and ammoType ~= -1 then
					...
					if amount == 0 then
						caller:ChatPrint("Üzgünüm, bu silah universal ammo kullanamıyor!")
					else
						caller:GiveAmmo(amount, ammo)
						self:Remove()
					end
				else
					caller:ChatPrint("Silahınıza mermi almak için elinizde silahınızı tutun!")
					caller.UniversalAmmoCooldown = CurTime()
				end
			end
		end
			...
			if swep and IsValid(swep) then
				...
				if ammoType and ammoType ~= -1 then
					...
					if not swep.IsUniversalAmmoInfinite then
						...
						caller:ChatPrint("Silah zaten infinite ammo ile donatılmış!")
					end
				else
					caller:ChatPrint("Elinizdeki silah primary ammo kullanamıyor!")
					caller.UniversalAmmoCooldown = CurTime()
				end
			end
		end
