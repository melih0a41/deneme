...
function PANEL:RCDSetScale(number)
    self.RCDScale = number
    self.RCDScale = number
end
...
function PANEL:Think()
    ...
    if isnumber(self.RCDScale) then
        self.RCDLerpScale = Lerp(FrameTime()*3, self.RCDLerpScale, self.RCDScale)
    
        if IsValid(self.Entity) then
            self.Entity:SetModelScale(self.RCDLerpScale)
        end
    end
    ...
    if input.IsMouseDown(MOUSE_LEFT) && self.RCDCanRotate then
        if not self.forceRotate then
            if not LocalPlayer():RCDCanAccessVehicle(RCD.ClientTable["vehicleId"]) then return end
        end
...
function PANEL:SetParams(paramsTbl, ent, bool, withoutLerp)
    ...
    if isnumber(paramsTbl["scale"]) then
        if withoutLerp then
            ent:SetModelScale(paramsTbl["scale"])
            self.RCDLerpScale = paramsTbl["scale"]
            self.RCDScale = paramsTbl["scale"]
        else
            self:RCDSetScale(paramsTbl["scale"])
        end
    end
