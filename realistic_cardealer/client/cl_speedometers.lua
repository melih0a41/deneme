...
hook.Add("HUDPaint", "RCD:HUDPaint:Speedometers", function()
    ...
    local size = RCD.ScrH*0.0009*speedoSize

    RCD.MaskStencil(function()
        RCD.DrawCircle(posx, posy, size*0.73, 0, 180, RCD.Colors["white"])
        for i=1, speedoCount do
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- 2ba374d8d8829bd100d8c29d24d3ea738b93520438d2999bc3fe661ec2953113
    ...
    if RCD.GetSetting("engineActivate", "boolean") && isfunction(vehc.GetDriver) && (vehc:GetDriver() == RCD.LocalPlayer) && not RCD.GetVehicleParams(vehicleId, "disableEngineVehicle") then
        ...
        RCD.DrawCircle(posEngineX, posEngineY, sizeEngine, 0, 360, RCD.Colors["white80248"])
        RCD.DrawCircle(posEngineX, posEngineY, lerpEngine, 0, 361, RCD.Colors["purple"])

        lerpEngineRed = Lerp(FrameTime()*5, lerpEngineRed, (redEngine and sizeEngine or 0))

        RCD.DrawCircle(posEngineX, posEngineY, lerpEngineRed, 0, 361, RCD.Colors["speedoRed"])
    ...
    if beltActivate then
        ...
        RCD.DrawCircle(posBeltX, posBeltY, sizeBelt, 0, 360, RCD.Colors["white80248"])
        RCD.DrawCircle(posBeltX, posBeltY, lerpBelt, 0, 361, RCD.Colors["purple"])
