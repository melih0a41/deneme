...
function RCD.NPCMenu(npcId, infoTable)
    ...
    local disableGarage = vgui.Create("DButton", npcSettings)
    disableGarage:SetSize(0, RCD.ScrH*0.048)
    disableGarage:Dock(TOP)
    disableGarage:DockMargin(0, 0, 0, RCD.ScrH*0.005)
    disableGarage:SetText("")
    disableGarage.Paint = function(self, w, h)
        draw.RoundedBox(0, 0, 0, w, h, RCD.Colors["white5"])
        draw.SimpleText(RCD.GetSentence("disableGarage"), "RCD:Font:13", w*0.015, h*0.5, RCD.Colors["white100"], TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end

    local checkBoxGarage = vgui.Create("RCD:CheckBox", disableGarage)
    checkBoxGarage:SetSize(RCD.ScrH*0.023, RCD.ScrH*0.023)
    checkBoxGarage:SetPos(RCD.ScrW*0.24,  disableGarage:GetTall()/2 - checkBoxGarage:GetTall()/2)
    checkBoxGarage.OnChange = function()
        local active = checkBoxGarage:GetActive()

        RCD.npcInformation["disableGarage"] = active
    end

    disableGarage.DoClick = function()
        local active = checkBoxGarage:GetActive()
        
        RCD.npcInformation["disableGarage"] = !active

        checkBoxGarage:SetActive(RCD.npcInformation["disableGarage"])
    end

    if infoTable && infoTable["disableGarage"] then
        RCD.npcInformation["disableGarage"] = infoTable["disableGarage"]

        checkBoxGarage:SetActive(infoTable["disableGarage"])
    end

    local disableShop = vgui.Create("DButton", npcSettings)
    disableShop:SetSize(0, RCD.ScrH*0.048)
    disableShop:Dock(TOP)
    disableShop:DockMargin(0, 0, 0, RCD.ScrH*0.005)
    disableShop:SetText("")
    disableShop.Paint = function(self, w, h)
        draw.RoundedBox(0, 0, 0, w, h, RCD.Colors["white5"])
        draw.SimpleText(RCD.GetSentence("disableShop"), "RCD:Font:13", w*0.015, h*0.5, RCD.Colors["white100"], TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end

    local checkBoxShop = vgui.Create("RCD:CheckBox", disableShop)
    checkBoxShop:SetSize(RCD.ScrH*0.023, RCD.ScrH*0.023)
    checkBoxShop:SetPos(RCD.ScrW*0.24,  disableShop:GetTall()/2 - checkBoxShop:GetTall()/2)
    checkBoxShop.OnChange = function()
        local active = checkBoxShop:GetActive()

        RCD.npcInformation["disableShop"] = active
    end

    disableShop.DoClick = function()
        local active = checkBoxShop:GetActive()
        
        RCD.npcInformation["disableShop"] = !active

        checkBoxShop:SetActive(RCD.npcInformation["disableShop"])
    end

    if infoTable && infoTable["disableShop"] then
        RCD.npcInformation["disableShop"] = infoTable["disableShop"]

        checkBoxShop:SetActive(infoTable["disableShop"])
    end
    ...
    saveNpc.DoClick = function()
        ...
        net.Start("RCD:Admin:Configuration")
            ...
            net.WriteBool(RCD.npcInformation["disableGarage"])
            net.WriteBool(RCD.npcInformation["disableShop"])
        net.SendToServer()
...
function RCD.NPCResellerMenu(npcId, infoTable)
    if IsValid(npcMenu) then npcMenu:Remove() end
    ...
    npcMenu.Paint = function(self,w,h)
        ...
        draw.DrawText(RCD.GetSentence("resellerConfigTitle"), "RCD:Font:10", w*0.045, h*0.013, RCD.Colors["white"], TEXT_ALIGN_LEFT)
        draw.DrawText(RCD.GetSentence("resellerDesc"), "RCD:Font:11", w*0.045, h*0.045, RCD.Colors["white100"], TEXT_ALIGN_LEFT)
    end
...
function RCD.CreateVehicle(editVehicle)    
    ...
    vehicleModel.PaintOver = function()
        RCD.GenerateWheels(vehicleModel.Entity, RCD.vehicleConfig["class"])
    end
    ...
    if editVehicle then
        local price = tostring(editVehicle["price"])
    ...
    createVehc.DoClick = function()
        ...
        net.Start("RCD:Admin:Configuration")
            ...
            net.WriteString(tostring(vehiclePrice))
...
function RCD.Settings()    
    ...
    if RCD && RCD.ParametersConfig && RCD.ParametersConfig["compatibilities"] && #RCD.ParametersConfig["compatibilities"] > 0 then
    ...
    local insuranceConfig = vgui.Create("RCD:Accordion", scrollConfig)
    insuranceConfig:SetSize(RCD.ScrW*0.538, RCD.ScrH*0.03)
    insuranceConfig:Dock(TOP)
    insuranceConfig:DockMargin(0, 0, 0, RCD.ScrH*0.006)
    insuranceConfig:SetText("insuranceSettings")
    insuranceConfig:InitializeCategory("insuranceSettings", nil, false, nil)
    ...
    elseif uInt == 8 then
        ...
        if class == "rcd_cardealer" then
            ...
            for i=1, groupsCount do
                ...
                local groupActivate = net.ReadBool()

                npcGroups[groupsId] = groupActivate
            end
            ...
            RCD.NPCResellerMenu(npcId, infoTable)
        end
