...
function MinigameObject:AddNewVar(VarName, VarType, Default)
    ...
    if ( Default ~= nil ) then
        ...
        Minigames.ThrowError([[The third argument of AddNewVar() must not be nil.]], Default, VarType)
    end
    ...
    elseif ( VarType == "string" ) then
        self:Checker(Default, "string", 3)

        self["Set" .. VarName] = function(SubSelf, Value)
            SubSelf:Checker(Value, "string", 1)
            SubSelf.__CustomVars[VarName] = Value
    ...
    if ( Result ~= true ) then
        Minigames.ThrowError([[There was an error when creating the variable.]], VarType, "boolean, number, or string")
    end
end
