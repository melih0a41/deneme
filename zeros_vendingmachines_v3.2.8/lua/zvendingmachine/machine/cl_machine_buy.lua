...
function zvm.Machine.BuyInterface(Machine)
	...
	btn_buy.Paint = function(s, w, h)
		...
		if Machine.BuyCost > 0 then
			draw.DrawText(zvm.Money.Display(Machine.BuyCost, zvm.Money.GetSymbol(Machine.MoneyType)), zclib.GetFont("zvm_interface_font01"), 70 / zvm.Machine.util.sm, 15 / zvm.Machine.util.sm, color_white, TEXT_ALIGN_CENTER)
...
function zvm.Machine.ProductList(Machine)
	...
	for k, v in pairs(Machine.Products) do
		...
		local CustomUpdate = zvm.Module.ProductImageOverwrite(v.class, v, img_pnl)
		...
		price_pnl.Paint = function(s, w, h)
			...
			draw.SimpleText(zvm.Money.Display(v.price,zvm.Money.GetSymbol(Machine.MoneyType)), zclib.GetFont("zvm_interface_item"), w / 2, h / 2, color_white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		end
...
function zvm.Actions.BuyList_AddItem(Machine,productid)
	...
	local BlockItem = zvm.Module.BlockItem(itemdata.class, itemdata, LocalPlayer(), Machine, productid)
	if BlockItem then return end
