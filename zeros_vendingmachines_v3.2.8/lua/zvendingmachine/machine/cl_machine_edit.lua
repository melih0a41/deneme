...
function zvm.Machine.ConfigInterface(Machine)
	...
	if Machine:GetPublicMachine() == false then
		...
		local Earnings_Payout = zvm.Machine.util.TextButton(Machine.VGUI.Content,zvm.Money.Display(Machine:GetEarnings() or 0, zvm.Money.GetSymbol(Machine.MoneyType)),function(btn)
			if Machine:GetEarnings() > 0 then
				zvm.Actions.RequestPayout(Machine)
				btn.Text = zvm.Money.Display(0,zvm.Money.GetSymbol(Machine.MoneyType))
			end
...
function zvm.Machine.EditProductList(Machine)
	...
	for k, v in pairs(Machine.Products) do
		...
		itm_pnl.Paint = function(s, w, h)
			...
			draw.SimpleText(zvm.Money.Display(v.price,zvm.Money.GetSymbol(Machine.MoneyType)), zclib.GetFont("zvm_interface_item"), 255 / zvm.Machine.util.sm, h / 2, color_white, TEXT_ALIGN_RIGHT,TEXT_ALIGN_CENTER)
		...
		local CustomUpdate = zvm.Module.IdleImageOverwrite(v.class, v, imgpnl)
