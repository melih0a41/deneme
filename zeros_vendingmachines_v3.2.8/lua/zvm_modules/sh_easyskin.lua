...
zvm.Definition.Add("easyskin", {
	ProductImageOverwrite = function(img_pnl, ItemData)
		if ItemData.skinpath and CL_EASYSKINS and IsValid(img_pnl) then
			...
			if image then
				img_pnl:SetMaterial(image)
			end

			return true
		end
	end,
	IdleImageOverwrite = function(img_pnl, ItemData)
		if ItemData.skinpath and CL_EASYSKINS and IsValid(img_pnl) then
			local image = CL_EASYSKINS.VMTToUnlitGeneric(ItemData.skinpath)
			...
			local BGLabel = vgui.Create("DPanel", img_pnl)
	...
	ItemUnpackOverwrite = function(ply, Crate, ItemData)
		if CL_EASYSKINS and IsValid(ply) then
	...
	BlockItem = function(ply, Machine, ItemData, ItemID)
		if not SH_EASYSKINS then return true end
	...
	ModifyProductDataOnPurchase = function(ply, ItemData)
		if not ItemData then return end
		if not SH_EASYSKINS then return true end
		local swep = ply:GetActiveWeapon()

		if IsValid(swep) then
			ItemData.extraData.weaponclass = swep:GetClass()
