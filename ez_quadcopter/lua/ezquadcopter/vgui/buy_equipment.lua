...
AccessorFunc(PANEL, "equipment", "Equipment")
...
function PANEL:LoadInterface()
    ...
    -- Don't display the equipment if already on the quadcopter
    if quadcopter.equipments[equipment.key] then
        action:SetText(easzy.quadcopter.languages.remove)
        action.DoClick = function()
            easzy.quadcopter.RemoveEquipment(quadcopter, equipment.key)
            self:Remove()
        end
    else
        action:SetText(easzy.quadcopter.languages.buy .. " " .. easzy.quadcopter.FormatCurrency(equipment.price))
        action.DoClick = function()
            easzy.quadcopter.BuyEquipment(quadcopter, equipment.key)
            self:Remove()
        end
    end
end
...
vgui.Register("EZQuadcopterBuyEquipment", PANEL, "EZQuadcopterFrame")
