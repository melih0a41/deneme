...
function ENT:Initialize()
	...
	-- Ghost mode için collision group ayarı
	self:SetupGhostCollisions()
...
-- Ghost mode collision ayarları
function ENT:SetupGhostCollisions()
	-- Ghost mode aktifse özel collision ayarları
	if self.upgrades and self.upgrades["Ghost"] and self.upgrades["Ghost"] > 0 then
		self:SetCollisionGroup(COLLISION_GROUP_DEBRIS_TRIGGER) -- Proplarla çarpışmaz
		
		-- Custom collision bounds (daha küçük)
		local min = Vector(-8, -8, -4)
		local max = Vector(8, 8, 4)
		self:SetCollisionBounds(min, max)
	else
		self:SetCollisionGroup(COLLISION_GROUP_NONE) -- Normal collision
	end
end
...
function ENT:Think()
	...
	-- Ghost mode collision güncelleme
	if self.lastGhostState != self.upgrades["Ghost"] then
		self:SetupGhostCollisions()
		self.lastGhostState = self.upgrades["Ghost"]
	end
...
function ENT:Collide(_, data)
	...
	if collisionSpeed > (100 * resistance) and speedDifference > (80 * resistance) and not collisionIsVertical then
		self:Break()
	end
end