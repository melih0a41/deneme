...
function FIB.CreateMainMenu()
    ...
    -- GÖREV PANELİ - GÖREV YÖNETİM BUTONLARI İLE
    ...
    if LocalPlayer().FIBRank == "Sef" or LocalPlayer().FIBRank == "Kidemli Ajan" then
        ...
        deleteMissionBtn.DoClick = function()
            ...
            if selected then
                ...
                local missionName = line:GetColumnText(1)
    ...
    addAgentBtn.DoClick = function()
        ...
        -- Online oyuncuları ekle - FIB.OnlineAgents kullanarak kontrol et
        for _, ply in ipairs(player.GetAll()) do
            local isAlreadyFIB = false
            for _, agent in ipairs(FIB.OnlineAgents or {}) do
                if agent.steamid == ply:SteamID() then
                    isAlreadyFIB = true
                    break
                end
            end
            
            if not isAlreadyFIB then
