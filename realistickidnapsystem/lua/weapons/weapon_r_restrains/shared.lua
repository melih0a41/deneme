...
-- OPTIMIZATION: Reduce Think frequency
SWEP.NextThink = 0
function SWEP:Think()
	-- Only process if we're actively restraining someone
	...
	elseif ct >= self.AttemptRestrainFinish then
		if SERVER then
			if self.RestrainingRagdoll then
...
if CLIENT then
	...
	function SWEP:Reload()
		...
		if TPlayer:GetNWBool("rks_restrained", false) then
			...
			OptionsMenu:SetPos(SW/2,SH/2)
		end
	end

	-- OPTIMIZATION: Cache colors and calculations
	local WHITE = Color(255,255,255,255)
	local BLACK = Color(0,0,0,255)
	local SW, SH = ScrW(), ScrH()
	
	hook.Add("OnScreenSizeChanged", "RKS_Restrains_UpdateCache", function()
		SW, SH = ScrW(), ScrH()
	end)

	function SWEP:DrawHUD()
		local y = 5
		draw.SimpleText("Left Click: Restrain player","default",SW/2,y,WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP,2,BLACK)
		draw.SimpleText("Right Click: Put dragged player in vehicle","default",SW/2,y+10,WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP,2,BLACK)
		draw.SimpleText("R: Inspect restrained player","default",SW/2,y+20,WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP,2,BLACK)
		draw.SimpleText("E: Drag restrained player (While dragging aim at prop/surface to attatch player)","default",SW/2,y+30,WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP,2,BLACK)
		...
		local TextToDisplay = string.format(RKS_GetLang("RestrainingText"), Nick) .. "(" .. TPercent .. "%)"
		draw.DrawText(TextToDisplay, "Trebuchet24", SW/2, SH/2, WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end