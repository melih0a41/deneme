...
-- ==========================================
-- OPTIMIZED: Think hook timer'a dönüştürüldü
-- ==========================================
timer.Create("TBFY_Surrender_Timer", 0.1, 0, function()
	for k,v in pairs(RKS_Surrendering) do
		local P, T = v.P, v.ST
		if IsValid(P) and v.ST < CurTime() then
		...
		elseif not IsValid(P) then
			-- Oyuncu geçersizse tablodan kaldır
			RKS_Surrendering[k] = nil
		end
	end
end)
