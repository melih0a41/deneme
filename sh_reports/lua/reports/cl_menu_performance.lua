...
function SH_REPORTS:ShowPerformanceReports()
	...
	local frame = self:MakeWindow(SH_REPORTS:L("performance_reports"))
	...
	_SH_REPORTS_PERF = frame
		...
		ilist_perf:AddColumn(SH_REPORTS:L("admin"))
		ilist_perf:AddColumn(SH_REPORTS:L("num_claimed"))
		ilist_perf:AddColumn(SH_REPORTS:L("num_closed"))
		ilist_perf:AddColumn(SH_REPORTS:L("time_spent"))
		...
		ilist_rating:AddColumn(SH_REPORTS:L("admin"))
		ilist_rating:AddColumn(SH_REPORTS:L("rating"))
		...
		ilist_history:AddColumn(SH_REPORTS:L("reporter"))
		ilist_history:AddColumn(SH_REPORTS:L("reported_player"))
		ilist_history:AddColumn(SH_REPORTS:L("reason"))
		ilist_history:AddColumn(SH_REPORTS:L("admin"))
		ilist_history:AddColumn(SH_REPORTS:L("rating"))
			...
			for _, info in pairs (history) do
				...
				reported:SetDrawBackground(false)
					...
					if (info.reported == "0") then
						...
						name:SetText("[" .. SH_REPORTS:L("other") .. "]")
				...
				local line = ilist_history:AddLine(reporter, reported, info.reason, admin, info.rating > 0 and rating or SH_REPORTS:L("none"))
				line:SetAlpha(0)
				line:AlphaTo(255, 0.1, 0.1 * i)
				line:SetToolTip(os.date(self.TimeFormat, info.date) .. "\n\n" .. SH_REPORTS:L("waiting_time") .. ": " .. self:FullFormatTime(info.waiting_time) .. "\n\n" .. SH_REPORTS:L("comment") .. ":\n" .. info.comment)
				self:LineStyle(line)
		...
		local function display_perf(start, prep)
			...
			frame.m_Title:SetText(SH_REPORTS:L("performance_reports") .. " (" .. ds .. " - " .. de .. ")")
			...
			self:Notify(SH_REPORTS:L("displaying_perf_report_from_x_to_y", ds, de), 5, styl.success, frame)
		end

		local btn_ratings = self:QuickButton(SH_REPORTS:L("rating"), function()
			ilist_perf:SetVisible(false)
			ilist_history:SetVisible(false)
			ilist_rating:SetVisible(true)
			ilist_rating:Clear()

			frame.m_Title:SetText(SH_REPORTS:L("performance_reports"))
			frame.m_Title:SizeToContentsX()
		...
		local btn_history = self:QuickButton(SH_REPORTS:L("history"), function()
			ilist_perf:SetVisible(false)
			ilist_rating:SetVisible(false)
