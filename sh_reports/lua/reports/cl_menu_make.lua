-- cl_menu_make.lua - TAM GÜNCELLENMİŞ HALİ
...
function SH_REPORTS:ShowMakeReports(c, d)
	...
	_SH_REPORTS_MAKE = frame
		...
		-- GERİ BUTONU - Sadece superadmin veya rapor görebilecek yetkililere göster
		if SH_REPORTS:IsAdmin(LocalPlayer()) then
			if not SH_REPORTS.OnlySuperadminCanSeeList or LocalPlayer():GetUserGroup() == "superadmin" then
		...
		body:DockMargin(m, m, m, m)
			...
			btns:DockMargin(0, m, 0, 0)
				...
				local submit = self:QuickButton(SH_REPORTS:L("submit_report"), function()
					...
					if (!steamid) then
						SH_REPORTS:Notify(SH_REPORTS:L("select_player_first"), nil, styl.failure, frame)
						return
					end

					local _, rid = reason:GetSelected()
					if (!rid) then
						SH_REPORTS:Notify(SH_REPORTS:L("select_reason_first"), nil, styl.failure, frame)
						return
					end
					
					-- Yetkili şikayeti kontrolü
					local reasonData = SH_REPORTS.ReportReasons[rid]
					if type(reasonData) == "table" and reasonData.disabled then
						SH_REPORTS:Notify("Bu kategori için rapor oluşturamazsınız. Discord üzerinden ticket açın.", nil, styl.failure, frame)
						return
					end
					...
					easynet.SendToServer("SH_REPORTS.NewReport", {
						...
						reported_id = steamid,
						reason_id = rid,
						comment = comment:GetValue():sub(1, SH_REPORTS.MaxCommentLength),
					})
			...
			reason.OnSelect = function(me, index, value, data)
				...
				if (d) then
					...
					if type(SH_REPORTS.ReportReasons[1]) == "table" then
						for k, v in pairs(SH_REPORTS.ReportReasons) do
					...
					local k = SH_REPORTS.ReasonAutoTarget[reasonText]
					if (!k) then
						return end
