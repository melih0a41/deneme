...
-- Alternatif: Oyuncu sit'ten ayrıldığında kontrol
hook.Add("GAS.AdminSits.SitLeft", "SH_REPORTS.GAS_PlayerLeftSit", function(ply)
    if IsValid(ply) and SH_REPORTS:IsAdmin(ply) then
        -- Bu admin'in aktif raporlarını kontrol et
        for id, report in pairs(SH_REPORTS.ActiveReports) do
            if report.admin_id == ply:SteamID64() then
                -- Raporu kapat
                SH_REPORTS:CloseReport(ply, id)
                SH_REPORTS:Notify(ply, "Adminsit'ten ayrıldığınız için rapor otomatik kapatıldı.", true)
                break
            end
        end
    end
...
-- Debug komutları
concommand.Add("sh_reports_debug_sits", function(ply)
    if not SH_REPORTS:IsAdmin(ply) then return end
    
    print("=== Aktif Raporlar ===")
    for id, report in pairs(SH_REPORTS.ActiveReports) do
        print("Rapor #" .. id)
        print("  Admin: " .. (report.admin_id or "yok"))
        print("  GAS Sit ID: " .. tostring(report.gas_sit_id))
    end
    
    if GAS and GAS.AdminSits then
        print("\n=== GAS AdminSits Durumu ===")
        print("GAS.AdminSits mevcut!")
        
        if GAS.AdminSits.ActiveSits then
            print("Aktif sit sayısı: " .. GAS.AdminSits.ActiveSits:len())
        end
    end
end)
