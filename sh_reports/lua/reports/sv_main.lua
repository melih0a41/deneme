if (!SH_REPORTS.ActiveReports) then
	...
	SH_REPORTS.ActiveSits = {} -- YENİ: Aktif sit listesi
end
...
function SH_REPORTS:FindAvailableSitLocation()
	...
	for _, location in ipairs(self.SitLocations) do
		...
		-- Eğer bu lokasyon boşsa, ek kontrol yap (yakında oyuncu var mı?)
		if not isOccupied then
			local nearbyPlayers = false
			for _, ply in ipairs(player.GetAll()) do
				if IsValid(ply) and ply:GetPos():Distance(location.pos) < self.SitAreaRadius then
					-- Sit'e dahil olmayan yakın oyuncu varsa bu alanı kullanma
					local inSit = false
					for _, sitInfo in pairs(self.ActiveSits) do
	...
	-- Tüm lokasyonlar doluysa ilkini döndür (zorunlu durum)
	return self.SitLocations[1]
end

-- YENİ: Oyuncuları sit alanına ışınla
function SH_REPORTS:TeleportPlayersToSit(players, location)
	...
	-- Oyuncuları daire şeklinde yerleştir
...
function SH_REPORTS:UnregisterSit(reportId)
	if self.ActiveSits[reportId] then
		-- Oyuncuları eski yerlerine gönder
...
function SH_REPORTS:HasActiveSit(admin)
	-- Kendi sit sistemimizi kontrol et
...
function SH_REPORTS:StartDelayedSit(admin, report)
	-- Base Kontrolü (ID: 7) için otomatik sit başlatma
	if report.reason_id == 7 then
		return -- Base kontrolü için otomatik sit yok
	end
	...
	timer.Create(timerName, 5, countdown / 5, function()
		...
		if countdown <= 0 then
			...
			self:Log(admin:Nick() .. " <" .. admin:SteamID() .. "> started auto-sit for report #" .. report.id .. " at " .. location.name)
		else
			-- Uyarı mesajları
			for _, ply in ipairs(players) do
				if IsValid(ply) then
...
function SH_REPORTS:ClaimAndTeleport(admin, id, bring, bring_reported)
	...
	-- Base Kontrolü (ID: 7) için özel durum - otomatik sit başlatma
	local isBaseControl = (report.reason_id == 7)
	
	-- Raporu üstlen
	local oldAutoStart = self.AutoStartSit
	if isBaseControl then
		-- Base kontrolü için otomatik sit'i devre dışı bırak
	...
	-- Base kontrolü için özel log mesajı
	if isBaseControl then
...
function SH_REPORTS:ClaimAndCSit(admin, id)
	-- YENİ: Kendi sit sistemimizi kullan
	if (!self:IsAdmin(admin)) then
		self:Notify(admin, "not_allowed_to_run_cmd", false)
		return
	end
	...
	-- Oyuncuları sit alanına ışınla
	self:TeleportPlayersToSit(players, location)
	
	-- Sit'i kaydet
	self:RegisterSit(report.id, location, players)
	report.sit_started = true
	...
	self:Log(admin:Nick() .. " <" .. admin:SteamID() .. "> claimed " .. target:Nick() .. "'s <" .. target:SteamID() .. "> report [#" .. id .. "] and started a sit at " .. location.name)
end
...
function SH_REPORTS:CloseReport(ply, id)
	...
	if (self:IsAdmin(ply) and ((report.admin_id == "" and self.CanDeleteWhenUnclaimed) or report.admin_id == sid)) or (report.reporter_id == sid) then
		...
		-- YENİ: Kendi sit sistemimizi temizle
		...
		if (!report.is_admin or self.AdminReportsCount) then
			if (report.admin_id ~= "") then
				...
				if (self.RewardEnabled and IsValid(admin) and sid == report.admin_id) then
					-- Base Kontrolü (ID: 7) her zaman para verir, diğerleri sit kontrolüne tabi
					local isBaseControl = (report.reason_id == 7)
					
					-- SIT KONTROLÜ: Base kontrolü hariç, sadece sit başlatıldıysa para ver
					if (isBaseControl or report.sit_started or self.ActiveSits[id]) then
