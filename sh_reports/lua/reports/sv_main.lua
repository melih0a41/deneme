...
local function InitializeReportsTables()
	...
	-- YENİ: GÜVEN SKORU CACHE TABLOSU
	if not SH_REPORTS.TrustScoreCache then
		SH_REPORTS.TrustScoreCache = {}
		print("[SH_REPORTS] TrustScoreCache tablosu oluşturuldu!")
	end
...
-- YENİ: GÜVEN SKORU FONKSİYONLARI
function SH_REPORTS:GetTrustScore(steamid)
	-- Cache kontrolü
	if self.TrustScoreCache[steamid] then
		return self.TrustScoreCache[steamid]
	end
	
	-- Varsayılan değer
	return self.DefaultTrustScore or 100
end

function SH_REPORTS:UpdateTrustScoreByRating(steamid, rating)
	if not self.TrustScoreEnabled then return end
	
	local change = self.TrustScoreChanges[rating] or 0
	local currentScore = self:GetTrustScore(steamid)
	local newScore = math.Clamp(currentScore + change, 0, 200)
	
	-- Cache güncelle
	self.TrustScoreCache[steamid] = newScore
	
	-- Veritabanına kaydet
	self:BetterQuery([[
		]] .. self.InsertSQL .. [[ sh_reports_trust_scores (steamid, score)
		VALUES ({steamid}, {score});
		UPDATE sh_reports_trust_scores SET score = {score}
		WHERE steamid = {steamid}
	]], {steamid = steamid, score = newScore})
	
	return newScore
end

function SH_REPORTS:LoadTrustScores()
	self:BetterQuery("SELECT * FROM sh_reports_trust_scores", {}, function(q, ok, data)
		if not ok then return end
		
		for _, row in pairs(data or {}) do
			self.TrustScoreCache[row.steamid] = tonumber(row.score) or self.DefaultTrustScore
		end
		
		print("[SH_REPORTS] " .. table.Count(self.TrustScoreCache) .. " güven skoru yüklendi.")
	end)
end

-- YENİ: SPAM KONTROLÜ
function SH_REPORTS:CanPlayerReport(ply)
	local sid = ply:SteamID64()
	
	-- Cooldown kontrolü
	if ply.LastReportTime and (os.time() - ply.LastReportTime) < self.ReportCooldown then
		local remaining = self.ReportCooldown - (os.time() - ply.LastReportTime)
		self:Notify(ply, string.format(self.CooldownMessage, remaining), false)
		return false, remaining
	end
	
	-- Güven skoru kontrolü
	if self.TrustScoreEnabled then
		local trust = self:GetTrustScore(sid)
		if trust < self.LowTrustThreshold then
			self:Notify(ply, "Düşük güven skorunuz nedeniyle rapor açamazsınız. Discord: discord.gg/basodark", false)
			return false
		end
	end
...
-- YENİ: AFK TIMER FONKSİYONLARI
function SH_REPORTS:StartAFKTimer(admin, reportId)
	if not self.AdminAFKTimeout then return end
	
	local timerName = "SH_REPORTS_AFK_" .. reportId
	
	-- Uyarı timer'ı
	timer.Create(timerName .. "_Warning", self.AdminAFKWarning, 1, function()
		if IsValid(admin) then
			self:Notify(admin, "DİKKAT: 1 dakika içinde işlem yapmazsanız rapor serbest kalacak!", false)
			admin:EmitSound("buttons/button10.wav")
		end
	end)
	
	-- Timeout timer'ı
	timer.Create(timerName, self.AdminAFKTimeout, 1, function()
		local report = self:FindReport(reportId)
		if report and report.admin_id == admin:SteamID64() then
			-- Raporu serbest bırak
			report.admin_id = ""
			report.last_activity = nil
			
			-- Bildirimleri gönder
			if IsValid(admin) then
				self:Notify(admin, "AFK olduğunuz için rapor serbest bırakıldı!", false)
			end
			
			-- Diğer adminlere haber ver
			easynet.Send(self:GetStaff(), "SH_REPORTS.AdminLeft", {report_id = reportId})
			
			self:Log("Report #" .. reportId .. " released due to admin AFK timeout")
		end
	end)
end

function SH_REPORTS:StopAFKTimer(reportId)
	timer.Remove("SH_REPORTS_AFK_" .. reportId)
	timer.Remove("SH_REPORTS_AFK_" .. reportId .. "_Warning")
end

function SH_REPORTS:UpdateActivity(admin, reportId)
	local report = self:FindReport(reportId)
	if report and report.admin_id == admin:SteamID64() then
		report.last_activity = os.time()
		
		-- Timer'ı yeniden başlat
		self:StopAFKTimer(reportId)
		self:StartAFKTimer(admin, reportId)
	end
end

function SH_REPORTS:DatabaseConnected()
	if (self.DatabaseMode == "mysqloo") then
		-- YENİ: GÜVEN SKORU TABLOSU
		self:Query("SHOW TABLES LIKE 'sh_reports_trust_scores'", function(q, ok, data)
			if (!ok) or (data and table.Count(data) > 0) then
				-- Tablo zaten var, skorları yükle
				self:LoadTrustScores()
			else
				-- Tabloyu oluştur
				self:Query([[
					CREATE TABLE `sh_reports_trust_scores` (
					  `steamid` varchar(64) COLLATE utf8_unicode_ci NOT NULL PRIMARY KEY,
					  `score` int(10) unsigned DEFAULT '100',
					  `last_update` int(10) unsigned DEFAULT '0'
					) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
				]], function(q2, ok2, data2)
					self:DBPrint("Creating sh_reports_trust_scores: " .. tostring(ok2))
					if ok2 then
						self:LoadTrustScores()
					end
				end)
		...
		self:Query("SHOW TABLES LIKE 'sh_reports_performance_history'", function(q, ok, data)
			...
			self:Query([[
				CREATE TABLE `sh_reports_performance_history` (
				  ...
				  `admin_rating` int(10) unsigned DEFAULT '0',
				  PRIMARY KEY (`id`)
				) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
			]], function(q2, ok2, data2)
		...
		-- YENİ: SQLite için güven skoru tablosu
		CreateTable("sh_reports_trust_scores", [[
		...
		CreateTable("sh_reports_performance_history", [[
			...
			`admin_rating` int(5) DEFAULT '0'
		]])

		self.InsertSQL = "INSERT OR IGNORE INTO"
		self:PostDatabaseConnected()
		
		-- Güven skorlarını yükle
		self:LoadTrustScores()
	end
end
...
-- GÜNCELLENDİ: Güven skoruna göre öncelik
function SH_REPORTS:GetAvailableReports()
		...
		if report.admin_id == "" then
			...
			if not isReserved then
				...
				if not (type(reasonData) == "table" and reasonData.disabled) then
					-- YENİ: Güven skoru bilgisini ekle
					report.trust_score = self:GetTrustScore(report.reporter_id)
	...
	-- Önceliğe ve güven skoruna göre sırala
	table.sort(reports, function(a, b)
		...
		-- YENİ: Düşük güven skorlu oyuncuların önceliğini düşür
		if self.LowTrustLowPriority then
			if a.trust_score < self.LowTrustThreshold then
				priorityA = priorityA + 10
			end
			if b.trust_score < self.LowTrustThreshold then
				priorityB = priorityB + 10
			end
		end
		
		if priorityA == priorityB then
			-- Aynı öncelikse güven skoruna bak
			if a.trust_score ~= b.trust_score then
				return a.trust_score > b.trust_score
			end
			-- Güven skoru da aynıysa zamana bak
...
function SH_REPORTS:PlayerSay(ply, str)
	...
	-- YENİ: Güven skoru komutu
	if (text == "/trustscore" or text == "!trustscore") then
		local score = self:GetTrustScore(ply:SteamID64())
		ply:ChatPrint("Güven skorunuz: " .. score .. "/200")
		if score < self.LowTrustThreshold then
			ply:ChatPrint("DİKKAT: Düşük güven skoru! Raporlarınız düşük öncelikli olacak.")
		end
		return ""
	end
end
...
-- GÜNCELLENDİ: Rapor üstlendikten sonra teleport işlemleri (AFK Timer ile)
function SH_REPORTS:TeleportToReport(admin, id, action)
	...
	-- YENİ: Aktivite güncelle
	self:UpdateActivity(admin, id)
...
-- GÜNCELLENDİ: Rapor için sit başlat (AFK Timer ile)
...
-- GÜNCELLENDİ: ClaimReport fonksiyonu (AFK Timer ile)
function SH_REPORTS:ClaimReport(admin, report)
	...
	report.admin_id = sid
	report.last_activity = os.time() -- YENİ: Son aktivite zamanı
	
	-- YENİ: AFK timer'ını başlat
	self:StartAFKTimer(admin, report.id)
...
function SH_REPORTS:StartDelayedSit(admin, report)
	...
	-- YENİ: Aktivite güncelle
	self:UpdateActivity(admin, report.id)
...
-- GÜNCELLENDİ: CloseReport (Admin rating sistemi ile)
function SH_REPORTS:CloseReport(ply, id, adminRating)
	local report = self:FindReport(id)
	if (!report) then
	...
	-- YENİ: Eğer admin raporu kapatıyorsa ve rating verilmediyse, rating penceresi aç
	if self:IsAdmin(ply) and report.admin_id == sid and not adminRating then
		-- Client'a rating penceresi açma mesajı gönder
		easynet.Send(ply, "SH_REPORTS.ShowReportRating", {
			report_id = id,
	...
	if (self:IsAdmin(ply) and ((report.admin_id == "" and self.CanDeleteWhenUnclaimed) or report.admin_id == sid)) or (report.reporter_id == sid) then
		...
		-- YENİ: Güven skoru güncelleme
		if isAdminClosing and adminRating then
			local newScore = self:UpdateTrustScoreByRating(report.reporter_id, adminRating)
			
			-- Admin'e bildirim
			local changeText = self.TrustScoreChanges[adminRating] or 0
			local ratingText = ""
			if adminRating == 5 then ratingText = "Tamamen haklı"
			elseif adminRating == 4 then ratingText = "Haklı"
			elseif adminRating == 3 then ratingText = "Kısmen haklı"
			elseif adminRating == 2 then ratingText = "Gereksiz"
			elseif adminRating == 1 then ratingText = "Troll/Spam"
			end
			
			easynet.Send(ply, "SH_REPORTS.Chat", {
				msg = "Rapor değerlendirmesi: " .. ratingText .. " (" .. (changeText >= 0 and "+" or "") .. changeText .. " güven puanı)"
			})
			...
			if IsValid(reporter) then
				easynet.Send(reporter, "SH_REPORTS.Chat", {
					msg = "Raporunuz '" .. ratingText .. "' olarak değerlendirildi. Yeni güven skorunuz: " .. newScore
				})
			end
		end
		
		-- AFK timer'ını durdur
		self:StopAFKTimer(id)
		...
		-- YENİ: Admin rating sonrası oyuncu rating'i
		if isAdminClosing and report.admin_id ~= "" then
			local reporter = player.GetBySteamID64(report.reporter_id)
			
			-- Oyuncu online ise admin'i puanlaması için pencere gönder
			if IsValid(reporter) and self.AskRating then
				if not reporter.SH_ReportsCompleted then
					reporter.SH_ReportsCompleted = {}
				end
				reporter.SH_ReportsCompleted[id] = report.admin_id
				
				-- Admin'in ismini al ve rating penceresini gönder
				local adminPlayer = player.GetBySteamID64(report.admin_id)
				local adminName = IsValid(adminPlayer) and adminPlayer:Nick() or "Yetkili"
				
				timer.Simple(0.5, function() -- Küçük bir gecikme ile gönder
					if IsValid(reporter) then
						easynet.Send(reporter, "SH_REPORTS.PromptRating", {
							report_id = id,
							admin_name = adminName
						})
						
						easynet.Send(reporter, "SH_REPORTS.Chat", {
							msg = "Raporunuz kapatıldı. Lütfen yetkilinin performansını değerlendirin."
						})
					end
				end)
			end
		...
		if (!report.is_admin or self.AdminReportsCount) then
			if (report.admin_id ~= "") then
				...
				-- PARA ÖDÜLÜ - GÜNCELLENDİ: Reporter çıkmış olsa bile verilecek
				local admin = player.GetBySteamID64(report.admin_id)
				if (self.RewardEnabled and IsValid(admin) and sid == report.admin_id) then
					...
					elseif report.sit_started or self.ActiveSits[id] then
						shouldGiveMoney = true
					-- YENİ: Rapor eden çıkmış olsa bile, admin işlem yaptıysa para ver
					elseif report.reporter_left then
						shouldGiveMoney = true
				...
				if (sid == report.reporter_id) then
					...
					-- YENİ: Oyuncuya admin'i puanlama penceresi gönder
					if (IsValid(target) and self.AskRating and report.admin_id ~= "") then
						if (!target.SH_ReportsCompleted) then
							target.SH_ReportsCompleted = {}
						end
						target.SH_ReportsCompleted[id] = ply:SteamID64()

						-- Oyuncuya rating penceresi gönder
						easynet.Send(target, "SH_REPORTS.PromptRating", {
							report_id = id, 
							admin_name = ply:Nick()
						})
						
						-- Oyuncuya bildirim
						easynet.Send(target, "SH_REPORTS.Chat", {
							msg = "Raporunuz kapatıldı. Lütfen yetkilinin performansını değerlendirin."
						})
					end
		...
		if (report.admin_id ~= "" and self.StoreCompletedReports ~= "none") then
			...
			self:BetterQuery([[
				]] .. self.InsertSQL .. [[ sh_reports_performance_history (id, reporter, reported, reason, comment, waiting_time, date, admin, admin_rating)
				VALUES ({id}, {reporter}, {reported}, {reason}, {comment}, {waiting_time}, {date}, {admin}, {admin_rating});
			]], {id = id, reporter = report.reporter_id, reported = report.reported_id, reason = reasonText, comment = report.comment, waiting_time = os.time() - report.time, date = os.time(), admin = report.admin_id, admin_rating = adminRating or 0})
		end
...
-- KRİTİK BUG DÜZELTMESİ: PlayerDisconnected
function SH_REPORTS:PlayerDisconnected(ply)
	...
	for id, report in pairs (self:GetAllReports()) do
		if (report.reporter_id == sid) then
			-- YENİ: RAPOR SİLİNMEYECEK, SADECE İŞARETLENECEK
			if report.admin_id ~= "" then
				-- Üstlenilmiş rapor - silme, sadece işaretle
				report.reporter_left = true
				
				local admin = player.GetBySteamID64(report.admin_id)
				if (IsValid(admin)) then
					self:Notify(admin, "Rapor eden oyuncu sunucudan ayrıldı! Raporu yine de kapatabilirsiniz.", false)
					easynet.Send(admin, "SH_REPORTS.Chat", {
						msg = "Rapor eden oyuncu çıktı ama rapor hala aktif. Kapatırsanız para ödülü alırsınız."
					})
				end
			else
				-- Üstlenilmemiş rapor - sil
				timer.Remove("SH_REPORTS_Timeout_" .. id)
				self.ActiveReports[id] = nil
				easynet.Send(self:GetStaff(), "SH_REPORTS.ReportClosed", {report_id = id})
			end
		elseif (self:IsAdmin(ply) and report.admin_id == sid) then
			-- Admin çıktı, raporu serbest bırak
			report.admin_id = ""
			report.last_activity = nil
...
-- GÜNCELLENDİ: NewReport (Spam kontrolü ve reported bildirimi ile)
function SH_REPORTS:NewReport(ply, data)
	...
	-- YENİ: Spam kontrolü
	local canReport, remaining = self:CanPlayerReport(ply)
	...
	-- YENİ: Spam koruması için zaman kaydet
	ply.LastReportTime = os.time()

	self:Notify(ply, "report_submitted", true)
	
	-- YENİ: Reported oyuncuya bildirim
	if self.NotifyReportedPlayer and IsValid(target) then
		easynet.Send(target, "SH_REPORTS.Chat", {
			msg = "Hakkınızda bir rapor oluşturuldu. Sebep: " .. (type(reasonData) == "table" and reasonData.reason or reasonData)
		})
		
		if self.ReportedPlayerSound then
			target:EmitSound(self.ReportedPlayerSound)
		end
	end
...
-- YENİ: Network Callbacks
easynet.Callback("SH_REPORTS.CloseReportWithRating", function(data, ply)
	SH_REPORTS:CloseReport(ply, data.report_id, data.admin_rating)
end)
...
concommand.Add("sh_reports_debug", function(ply)
	...
	for id, report in pairs(SH_REPORTS.ActiveReports) do
		...
		-- YENİ: Güven skoru ve reporter left durumu
		local trustScore = SH_REPORTS:GetTrustScore(report.reporter_id)
		local leftStatus = report.reporter_left and " [OYUNCU ÇIKTI]" or ""
		
		print("Rapor #" .. id .. reserved .. leftStatus)
		print("  Rapor Eden: " .. report.reporter_name .. " (Güven: " .. trustScore .. ")")
	...
	print("\n=== Güven Skorları (Cache) ===")
	for steamid, score in pairs(SH_REPORTS.TrustScoreCache) do
		print(steamid .. ": " .. score)
	end
end)
