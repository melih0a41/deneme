...
function SH_REPORTS:PlayerSay(ply, str)
	...
	-- Sıradaki rapor komutu
	if (text == self.NextReportCommand:lower()) then
...
function SH_REPORTS:AssignNextReport(admin)
	...
	if self:ClaimReport(admin, nextReport) then
		...
		local reasonData = self.ReportReasons[nextReport.reason_id]
		local reasonText = reasonData
		if type(reasonData) == "table" then
			reasonText = reasonData.reason
		end
		
		local waitTime = self:FullFormatTime(os.time() - nextReport.time)
		
		chat.AddText(
			self.Style.header, "[", self:L("reports"), "] ",
			Color(255, 255, 255), "Rapor #", nextReport.id, " size atandı:\n",
			Color(100, 255, 100), "Rapor Eden: ", Color(255, 255, 255), nextReport.reporter_name, "\n",
			Color(100, 255, 100), "Şikayet Edilen: ", Color(255, 255, 255), nextReport.reported_name, "\n", 
			Color(100, 255, 100), "Sebep: ", Color(255, 255, 255), reasonText, "\n",
			Color(100, 255, 100), "Bekleme Süresi: ", Color(255, 255, 255), waitTime, "\n",
			Color(255, 200, 100), "30 saniye sonra sit başlayacak!"
		)
		
		-- Rapor detaylarını aç
		timer.Simple(0.5, function()
			if IsValid(admin) then
				easynet.Send(admin, "SH_REPORTS.MinimizeReport", {report_id = nextReport.id})
			end
		end)
		
		-- Diğer bekleyen raporların sayısını bildir (yetkili şikayetleri hariç)
		if #prioritizedReports > 1 then
			chat.AddText(
				self.Style.header, "[", self:L("reports"), "] ",
				Color(255, 200, 100), "Sırada ", #prioritizedReports - 1, " rapor daha bekliyor."
			)
		end
	end
end
