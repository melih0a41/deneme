...
function SH_REPORTS:NewReport(ply, data)
	...
	if not (type(reasonData) == "table" and reasonData.disabled) then
		...
		-- Tüm yetkililere sadece bekleyen rapor sayısını bildir
		for _, admin in ipairs(player.GetAll()) do
			if IsValid(admin) and self:IsAdmin(admin) then
				-- SADECE bekleyen rapor sayısı bildirimi
...
function SH_REPORTS:NewReport(ply, data)
	...
	-- YENİ: Yetkili şikayeti değilse tüm yetkililere ANINDA chat bildirimi gönder
	if not (type(reasonData) == "table" and reasonData.disabled) then
		-- ANINDA bildirim gönder (timer.Simple kaldırıldı)
		local pendingCount = 0
		for id, report in pairs(self.ActiveReports) do
			if report.admin_id == "" then
				local rd = self.ReportReasons[report.reason_id]
		...
		-- Tüm yetkililere ANINDA bildir
		for _, admin in ipairs(player.GetAll()) do
			if IsValid(admin) and self:IsAdmin(admin) then
				-- Önce yeni rapor bildirimi
				easynet.Send(admin, "SH_REPORTS.Chat", {
					msg = " YENİ RAPOR! " .. ply:Nick() .. " tarafından " .. data.reported_name .. " hakkında: " .. reasonText
				})
				
				-- Sonra bekleyen rapor sayısı
...
function SH_REPORTS:StartReportTimeout(reportId)
	...
	timer.Create(timerName, 300, 6, function() -- 300 saniye = 5 dakika, 6 kez = 30 dakika
	...
	-- İlk mesajı gönder (5 dakika beklemeden)
	timer.Simple(1, function()
		...
		if hasStaff then
			easynet.Send(reporter, "SH_REPORTS.Chat", {
				msg = "Raporunuz sırada bekliyor. 30 dakika içinde işleme alınmazsa otomatik iptal edilecek."
			})
		else
			easynet.Send(reporter, "SH_REPORTS.Chat", {
