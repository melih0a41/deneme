			...
			actions:Dock(BOTTOM)
				...
				if (self:IsAdmin(LocalPlayer())) then
					if (report.admin_id == "") then
						-- Base Kontrolü (ID: 7) için özel durum
						if (report.reason_id == 7) then
							-- Base Kontrolü raporu - Git butonu göster
							...
							-- Info
							local info = self:QuickLabel("(Base kontrolü için)", "{prefix}Small", styl.text, actions)
							info:SetContentAlignment(4)
							info:Dock(LEFT)
							info:DockMargin(m2, 0, 0, 0)
						else
							-- Diğer raporlar için normal claim butonu
...
function SH_REPORTS:MakePending(report)
	-- POP-UP'LAR DEVRE DIŞI!
	-- Artık sadece chat bildirimleri kullanılıyor
	return
