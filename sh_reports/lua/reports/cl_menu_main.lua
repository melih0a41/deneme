		...
		body:Dock(FILL)
			...
			players:Dock(TOP)
				...
				lbl1.Paint = function(me, w, h)
					...
					-- YENİ: Reporter çıktıysa uyarı göster
					if report.reporter_left then
						nic:SetTextColor(styl.failure)
						nic:SetToolTip("Bu oyuncu sunucudan ayrıldı!")
					end
				...
				if (self:IsAdmin(LocalPlayer())) then
					...
					elseif (report.admin_id == LocalPlayer():SteamID64()) then
						...
						if report.reporter_left then
							...
							goto:SetToolTip("Oyuncu sunucudan ayrıldı!")
							bring:SetToolTip("Oyuncu sunucudan ayrıldı!")
							sit:SetToolTip("Oyuncu sunucudan ayrıldı!")
						end
				...
				if (report.reporter_id == LocalPlayer():SteamID64()) or 
				   ...
				   (report.admin_id == LocalPlayer():SteamID64()) then
					local close = self:QuickButton(SH_REPORTS:L("close_report"), function()
						-- YENİ: Eğer admin kapatıyorsa, önce rating penceresi açılacak
						if self:IsAdmin(LocalPlayer()) and report.admin_id == LocalPlayer():SteamID64() then
							-- Admin kapatıyor, oyuncu değerlendirmesi için rating iste
