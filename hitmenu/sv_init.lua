--[[
Hitman Sistemi - Sunucu Tarafı
/hitiptal komutu ve başkan koruması eklendi
--]]
...
-- YENİ KOMUT: /hitiptal
DarkRP.defineChatCommand("hitiptal", function(ply, args)
    if not hits[ply] then
        DarkRP.notify(ply, 1, 4, DarkRP.getPhrase("no_active_hit"))
        return ""
    end
...
function DarkRP.hooks:onHitAccepted(hitman, target, customer)
    ...
    local targetName = "Bilinmeyen Oyuncu"
...
hook.Add("PlayerDeath", "DarkRP Hitman System", function(ply, inflictor, attacker)
    -- Eğer ölen kişi bir hitman ise, hit iptal olur
    if hits[ply] then
        ply:abortHit(DarkRP.getPhrase("hitman_died"))
    end

    -- Eğer saldıran kişi hitman ise ve ölen kişi hedef ise, hit tamamlanır
