...
local function movePopups()
    ...
    -- DEĞİŞTİRİLDİ: Sol üst köşeye geri taşındı
    local baseX = padding
    ...
    for _, popup in ipairs( onyx.hud.popups ) do
        ...
        -- DEĞİŞTİRİLDİ: Aşağı doğru hizala (eski hali)
...
local function createPopup( title, desc, duration, acceptCallback, denyCallback, onRemove )
    ...
    -- Mouse input kontrolü - sadece gerektiğinde aktif
    ...
    -- Mouse alanını kontrol et - sadece popup üzerindeyken mouse input'u yakala
    popup.OnCursorEntered = function( panel )
        panel:SetMouseInputEnabled( true )
    end
    popup.OnCursorExited = function( panel )
    ...
    lblTitle:SetMouseInputEnabled( false ) -- Label'da mouse input yok
    ...
    lblDesc:SetMouseInputEnabled( false ) -- Label'da mouse input yok
    popup.lblDesc = lblDesc

    local conButtons = popup:Add( 'Panel' )
    conButtons:SetTall( buttonHeight )
    conButtons:Dock( BOTTOM )
    conButtons:SetMouseInputEnabled( true ) -- Butonlar için mouse input gerekli
