...
-- AimbotBreakerBot filtresi - Basitleştirilmiş versiyon (cl_core.lua'da ana kontrol var)
local function IsAimbotBreakerBot(ply)
    -- cl_core.lua'daki IsValid override'ı kullanacağız
...
local updateSlowLabels do
    ...
    function updateSlowLabels( client, maxWidth )
        if ( nextUpdate <= CurTime() ) then
            ...
            -- AimbotBreakerBot kontrolü ekledik
            if IsAimbotBreakerBot(client) then
                slowLabels.name = {
                    text = "Unknown Player",
                    font = hud.fonts.Name
                }
