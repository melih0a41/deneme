--[[
Dosya Konumu: onyx_hud/lua/onyx/modules/hud/elements/cl_main.lua
]]--
...
do
    ...
    updateSlowLabels = function( client, maxWidth )
        if ( nextUpdate <= CurTime() ) then
            nextUpdate = CurTime() + thinkRate
            
            local name = client:Name()
            
            -- DarkRP custom job kontrolü
            local job
            if DarkRP and client.getDarkRPVar then
                -- Önce custom job'u kontrol et
                local customJob = client:getDarkRPVar("job")
                if customJob and customJob ~= "" then
                    job = customJob
                else
                    -- Custom job yoksa normal mesleği al
                    local jobName = client.getJobName and client:getJobName() or team.GetName(client:Team())
                    job = jobName
                end
            else
                -- DarkRP değilse normal team ismini kullan
                job = team.GetName(client:Team())
            end
