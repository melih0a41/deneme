--[[
Dosya Konumu: onyx_hud/lua/onyx/modules/hud/elements/cl_overhead.lua
]]--
...
local function drawInfo( ply, client )
    ...
    if DarkRP and ply.getDarkRPVar then
        ...
        local customJob = ply:getDarkRPVar("job")
        if customJob and customJob ~= "" then
            teamName = customJob
        else
            -- Custom job yoksa normal mesleği al
            teamName = ply.getJobName and ply:getJobName() or team.GetName(teamID)
        end
    else
    ...
    if onyx.hud.GetPlayerVIPRank then
        vipRank, vipData = onyx.hud.GetPlayerVIPRank(ply)
    end
