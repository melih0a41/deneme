...
-- FİX: Oyuncu jail durumu kontrolü - FONKSİYON TANIMI
...
-- Kemik sıfırlama fonksiyonu - FONKSİYON TANIMI
...
local function EndRentalForPlayer(steamId, reason)
    ...
    -- Tablodan hemen kaldır (tekrar çağrılmasını önlemek için)
    gScooters.ActiveRentals[steamId] = nil
    
    -- Oyuncu geçerliyse işlemleri yap
    if IsValid(player) then
        -- Oyuncuyu scooter'dan çıkar
        if IsValid(scooter) and player:GetVehicle() == scooter then
            player:ExitVehicle()
        end
...
-- FİX: RentScooter network receiver - sv_net.lua'dan buraya taşındı
net.Receive("gScooters.Net.RentScooter", function(len, pPlayer)
    ...
    -- ÖNEMLİ: Detaylı mesafe kontrolü
    local playerPos = pPlayer:GetPos()
    local scooterPos = eScooter:GetPos()
    local distance = playerPos:Distance(scooterPos)
    local maxRentDistance = 150 -- Maksimum kiralama mesafesi (birim)
    
    DebugPrint("RentScooter distance check - Player:", pPlayer:Nick(), "Distance:", math.Round(distance))
    
    if distance > maxRentDistance then
        DebugPrint("Player too far from scooter! Distance:", math.Round(distance), "Max:", maxRentDistance)
        gScooters:ChatMessage("Scooter'dan çok uzaksınız! Lütfen yaklaşın.", pPlayer)
        
        -- Kemikleri sıfırla (yanlışlıkla uygulanmış olabilir)
        ...
        -- Tüm rental ile ilgili değişkenleri temizle
        ...
        -- ÖNEMLİ: Burada return ediyoruz, hiçbir şey yapılmayacak
        return
    end
    ...
    -- YENİ: Oyuncunun başka bir araçta olup olmadığını kontrol et
    ...
    -- YENİ: Zaten işlem yapılıyor mu kontrolü
    if pPlayer.GC_RentalProcessing then
        DebugPrint("Rental already processing for player")
        return
    end

    -- MESAFE KONTROLÜNÜ GEÇTİ - ŞİMDİ KİRALAMA BAŞLAYABİLİR
    DebugPrint("Player passed all checks, starting rental process")
    
    -- Geçici bir flag koy
    pPlayer.GC_RentalProcessing = true
    
    -- Unique timer ID oluştur
    local timerID = "GC_RentProcess_" .. pPlayer:SteamID64() .. "_" .. eScooter:EntIndex()
    ...
    timer.Create(timerID, 0.3, 1, function()
        ...
        if not IsValid(pPlayer) or not IsValid(eScooter) then
            DebugPrint("Entity became invalid")
            if IsValid(eScooter) then
        ...
        -- SON MESAFE KONTROLÜ
        local finalDistance = pPlayer:GetPos():Distance(eScooter:GetPos())
        if finalDistance > maxRentDistance * 1.5 then
            DebugPrint("Player too far in final check. Distance:", math.Round(finalDistance))
            eScooter.GC_Enterable = false
            ...
            gScooters:ChatMessage("Scooter'dan çok uzaksınız!", pPlayer)
            return
        end
        ...
        -- BAŞARIYLA BİNDİ - ŞİMDİ KİRAYI BAŞLAT
        ...
        -- Rental başladı hook'u
        hook.Run("gScooters.RentalStarted", pPlayer, eScooter)
        
        -- 5 saniye sonra scooter'ı aktifleştir
        local activateTimerID = "GC_ActivateRental_" .. pPlayer:SteamID64()
        timer.Create(activateTimerID, 5, 1, function()
            ...
            if pPlayer:GetVehicle() == eScooter then
                ...
                if gScooters.ActiveRentals[steamId] then
                    gScooters.ActiveRentals[steamId] = nil
                end
...
function gScooters:HandleEnd(eScooter, pPlayer)
    ...
    -- Aktif rental'ı tablodan kaldır
    local steamId = pPlayer:SteamID64()
    if gScooters.ActiveRentals[steamId] then
        gScooters.ActiveRentals[steamId] = nil
    end
...
hook.Add("PlayerLeaveVehicle", "gScooters.Hook.OnScooterExit", function(pPlayer, eScooter)
    if IsValid(eScooter) and eScooter.gScooter then
        ...
        -- Önce normal HandleEnd'i çağır
        gScooters:HandleEnd(eScooter, pPlayer)
        
        -- Eğer hala aktif rental varsa (HandleEnd sonlandırmadıysa), mesafe kontrolü yap
        if rentalData and rentalData.scooter == eScooter then
            -- 5 saniye bekle, eğer hala uzaktaysa rental'ı zorla sonlandır
            timer.Simple(5, function()
                if not IsValid(pPlayer) then return end
                
                local rental = gScooters.ActiveRentals[steamId]
                if not rental then return end -- Zaten sonlandırılmış
