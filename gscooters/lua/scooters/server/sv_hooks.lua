...
hook.Add("PlayerLeaveVehicle", "gScooters.Hook.OnScooterExit", function(pPlayer, eScooter)
    if IsValid(eScooter) and eScooter.gScooter then
        -- Pending rental kontrolü - eğer kira henüz aktif değilse
        if pPlayer.GC_PendingRental then
            DebugPrint("Player left scooter during pending rental, cancelling")
            
            -- Pending kirayı iptal et
            pPlayer.GC_ScooterRentStartTime = nil
            pPlayer.GC_ActiveScooter = nil
        ...
        -- Normal kira bitirme (5 saniye geçtiyse)
        if pPlayer.GC_ScooterRentStartTime and (CurTime() - pPlayer.GC_ScooterRentStartTime) > 5 then
            gScooters:HandleEnd(eScooter, pPlayer)
        elseif pPlayer.GC_ScooterRentStartTime then
            -- 5 saniye geçmemişse ama pending değilse (bu durum olmamalı normalde)
            pPlayer.GC_ScooterRentStartTime = nil
            pPlayer.GC_ActiveScooter = nil
            pPlayer.GC_PendingRental = nil
