...
function gScooters:BillPlayer(pPlayer, iTime)
    ...
    if gScooters:CanAfford(pPlayer, iPrice) then
        ...
        -- Para yetersiz - mevcut parayı al ve kira sonlandır
        local currentMoney = gScooters:GetMoney(pPlayer)
        if currentMoney > 0 then
            gScooters:ModifyMoney(pPlayer, -1 * currentMoney)
            gScooters:ChatMessage(string.format("Para yetersiz! Mevcut paranız (%s) alındı ve kira sonlandırıldı.", gScooters:FormatMoney(currentMoney)), pPlayer)
        else
            gScooters:ChatMessage("Para yetersiz! Kira sonlandırıldı.", pPlayer)
        end
        
        gScooters:ChatMessage(gScooters:GetPhrase("blacklist"), pPlayer)
        gScooters.PlayerBlacklists[pPlayer:SteamID64()] = CurTime()
        
        -- ÖNEMLİ: Kira bilgilerini temizle
