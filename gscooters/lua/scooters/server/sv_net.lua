...
net.Receive("gScooters.Net.RentScooter", function(len, pPlayer)
    ...
    -- Scooter'ı kiralanabilir yap ve oyuncuyu bindir
    eScooter.GC_Enterable = true
    pPlayer:EnterVehicle(eScooter)
    eScooter.GC_Enterable = false
    
    -- VCMod veya kemik ayarları - ORİJİNAL GİBİ HEMEN YAPILIYOR
    ...
    timer.Simple(1, function()
        if IsValid(pPlayer) and IsValid(eScooter) then
            if pPlayer:GetVehicle() == eScooter then
                ...
                -- 5 saniye sonra scooter'ı aç
                timer.Simple(4, function() -- 1 + 4 = 5 saniye toplam
                    if IsValid(pPlayer) and IsValid(eScooter) then
                        -- Önce oyuncunun hala scooter'da olup olmadığını kontrol et
                        if pPlayer:GetVehicle() == eScooter then
                            -- Hala scooter'da, kirayı aktifleştir
                            eScooter:Fire("TurnOn")
                            pPlayer.GC_PendingRental = nil -- Artık kira aktif
                            DebugPrint("Rental ACTIVATED for", pPlayer:Nick())
                        else
                            -- Oyuncu scooter'da değil, değişkenleri temizle
                            DebugPrint("Player not in scooter after 5 seconds, cleaning up")
                ...
                gScooters:ChatMessage("Scooter'a binilemedi, kiralama iptal edildi!", pPlayer)
