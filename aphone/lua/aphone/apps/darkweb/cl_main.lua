...
function APP:Open(main, main_x, main_y, screenmode)
    ...
    local function check_validentry()
        ...
        if c and c.config then
            local n = math.Clamp(tonumber(create_name_priceenter:GetText()), (c.config.min > 0) and c.config.min or 0, (c.config.max < 10000000000 and c.config.max > 0) and c.config.max or 10000000000)
            create_name_priceenter:SetText(n)
        end
    ...
    function create_name_priceenter:DoClick()
        local a = self:Phone_AskTextEntry("", 10)
...
function APP:ShowCondition()
	...
	return !c or !c.config or c.config.viewing_jobs[t] or
        c.config.killing_jobs[t]
end
