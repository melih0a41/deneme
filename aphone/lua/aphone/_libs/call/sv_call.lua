...
hook.Add("PlayerCanHearPlayersVoice", "aphone_VoiceManagement", canHear)
...
if VoiceBox and VoiceBox.FX then
	hook.Add("PlayerCanHearPlayersVoice", "aphone_VoiceManagement", voiceBoxCanHear)
else
...
hook.Add("SetupPlayerVisibility", "aphone_PVSOnFaceCam", function(p)
	if aphone.Call.Table[p.aphoneCallID] then
		...
		if !IsValid(t.ent1) or !IsValid(t.ent2) then
			end_call(p)
		else
