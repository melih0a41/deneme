...
function Minigames.IsAllowed(ply)
    ...
    if Minigames.Config["EverybodyCanUse"] then
        return true
    end

    return
        Minigames.Config["UseFunction"] and
        Minigames.Config["AllowUserFunction"]( ply ) or
        Minigames.Config["AllowUserGroup"][ ply:GetUserGroup() ]
end

function Minigames.IsAllowedToReward(ply)
    ply = CLIENT and LocalPlayer() or ply
    if not IsValid( ply ) then return false end

    if Minigames.Config["EverybodyCanReward"] then
        return true
    end
...
function Minigames.IsAllowedAdmin(ply)
    ply = CLIENT and LocalPlayer() or ply
    if not IsValid( ply ) then return false end
...
concommand.Add("minigames_reload", function(ply)
    if SERVER then
        ...
        if not Minigames.IsAllowedAdmin() then return end
