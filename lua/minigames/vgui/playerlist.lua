...
local ButtonPaint = function(self, w, h)
    ...
    if self:IsEnabled() then
        ...
        draw.RoundedBox(4, 0, 0, w, h, ButtonStyleDisabled)
    end
end
...
function PANEL:Init()
    ...
    self.BroadcastGameJoin.DoClick = function()
        ...
        WAITING = CurTime() + Minigames.Config["JoinGameCommandDelay"]
    ...
    local BroadcastCanBeEnabled = (
        ...
        not Minigames.GetOwnerGame( LocalPlayer() ):IsActive() and
        WAITING < CurTime()
    )
...
hook.Add("Minigames.PostRemoveGame", "Minigames.WaitingStop", function()
    WAITING = 0
end)