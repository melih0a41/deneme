...
function MinigameObject:AddPlayer(NewPlayer)
    ...
    if not IsValid( CurrentOwner ) then
        self.ThrowError("There is no owner for this minigame.", CurrentOwner, "player")
    end

    if not Minigames.IsAllowedAdmin(CurrentOwner) then
        return false
    end

    if self.__Players[NewPlayer] then
        Minigames.BroadcastMessage( Minigames.StringFormat( Minigames.GetPhrase("minigames.player.alreadyingame"), NewPlayer ), CurrentOwner )
        return false
    end
    ...
    if Minigames.Config["TeleportOnJoin"] then
        self:TeleportPlayer(NewPlayer)
    end
...
function MinigameObject:OnPlayerAdded(func)
    ...
    table.insert( self.__PlayerAddedHooks, func )
end
...
function MinigameObject:TeleportPlayers(Entities, Offset)
    if not Minigames.IsAllowedAdmin(self:GetOwner()) then return end
