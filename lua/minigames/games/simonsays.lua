...
function GameScript:FullyStartGame()
    ...
    local MainTimer = self:CreateTimer("Main")
    MainTimer:SetLoop(true)
    MainTimer:SetVariable({
        ["RandomColor"] = -1,
        ["Screens"] = self:GetAllEntities("Screens")
    })
...
function GameScript:StartGame()
    ...
    local BeginTimer = self:CreateTimer("Begin")
    BeginTimer:SetLoop(8)
    BeginTimer:SetVariable({
        ["Count"] = 1
    })
