...
SWEP.Instructions = "Para calmak icin hedefe sol klik tıkla (%10)"
SWEP.IconLetter = ""
	...
	if CLIENT then
		...
		timer.Create( "PickpocketDots", 0.5, 0, function() 
			if not self:IsValid() or not self.IsPickpocketing then
				timer.Destroy( "PickpocketDots" ); self.Dots = ""
				return
			end
...
function SWEP:Holster()
	...
	if CLIENT then timer.Destroy( "PickpocketDots" ); self.Dots = "" end
	return true
end
...
function SWEP:Succeed()
	...
	if SERVER then
		...
		local moneyToSteal = math.floor( targetMoney * 0.10 ) 
		...
		if targetMoney >= moneyToSteal and moneyToSteal > 0 then
            ...
            if IsValid(owner) then
                 owner:ChatPrint("[Kapkaç] Başarıyla " .. formattedMoney .. " TL çaldın!")
            end
            if IsValid(target) then
                 target:ChatPrint("[Kapkaç] Dikkat! " .. formattedMoney .. " TL paran çalındı!")
            end
        ...
        if IsValid(owner) then
             owner:ChatPrint("[Kapkaç] Bekleme süresi: " .. PPConfig_Wait .. " saniye.")
        end
	end
end
...
function SWEP:Fail()
    ...
    if CLIENT then
        if IsValid(owner) and owner == LocalPlayer() then 
		    owner:PrintMessage( HUD_PRINTTALK, "Kapkac basarisiz." )
        end
	end
...
function SWEP:Think()
	...
	-- YENİ: Daha esnek hedef kontrolü
	...
	-- YENİ: Maksimum mesafe kontrolü (başlangıç mesafesi + tolerans)
	local maxDistance = PPConfig_Distance + PPConfig_MovementTolerance
	...
	-- Eğer hedef çok uzaklaştıysa iptal et
...
function SWEP:DrawHUD()
	if self.IsPickpocketing and self.EndPick and IsValid(self.Owner) and self.Owner == LocalPlayer() then
		...
		draw.DrawNonParsedSimpleText( "Caliniyor" .. self.Dots, "Trebuchet24", w / 2, y + height / 2, Color( 255, 255, 255, 255 ), 1, 1 )
	end
end
