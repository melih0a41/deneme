-- SAM VIP Yönetim Sistemi - Client Menü (DÜZENLENMİŞ KOD)
...
function PANEL:Init()
    ...
    -- Tüm VIP'lere süre ekle butonu
    self.AddAllVIPBtn = vgui.Create("DButton", self.ButtonsPanel)
    self.AddAllVIPBtn:Dock(LEFT)
    self.AddAllVIPBtn:SetWide(200)
    self.AddAllVIPBtn:DockMargin(5, 0, 5, 0)
    self.AddAllVIPBtn:SetText("Tüm VIP'lere Süre Ekle")
    self.AddAllVIPBtn:SetFont("DermaDefault")
    self.AddAllVIPBtn:SetTextColor(Color(255, 255, 255))
    self.AddAllVIPBtn.Paint = function(s, w, h)
        draw.RoundedBox(4, 0, 0, w, h, Color(255, 193, 7, s:IsHovered() and 255 or 200))
    end
    self.AddAllVIPBtn.DoClick = function()
        self:OpenAddTimeToAllMenu()
    end
...
function PANEL:OpenAddVIPMenu()
    ...
    --[[ HATA DÜZELTMESİ: Buradaki fonksiyonun tamamı güncellendi ]]--
    addbtn.DoClick = function()
        ...
        if steamid and typeid and time ~= nil then -- 'time' 0 olabileceğinden nil kontrolü yapıldı
            local vipType = typeid:gsub("vip$", "")
            
            -- HATA DÜZELTMESİ: İsim çakışmalarını önlemek için takma ad yerine SteamID kullanılıyor.
            local target_identifier = "#" .. steamid
            
            if time == 0 then
                RunConsoleCommand("sam", "vipver", target_identifier, vipType, "0")
            else
                RunConsoleCommand("sam", "vipver", target_identifier, vipType, tostring(time))
            end
            
            frame:Close()
            timer.Simple(0.5, function()
                if IsValid(self) then self:RefreshList() end
            end)
        end
    end
end
...
function PANEL:CreateActionButtons(line, data)
    ...
    actions:SetWide(490) -- Eski genişlik
    actions:Dock(RIGHT)
...
function PANEL:OpenAddTimeToAllMenu()
    local frame = vgui.Create("DFrame")
    ...
    frame.Paint = function(s, w, h)
        draw.RoundedBox(12, 0, 0, w, h, Color(25, 25, 25, 240))
        draw.RoundedBoxEx(12, 0, 0, w, 45, Color(35, 35, 35, 255), true, true, false, false)
        draw.SimpleText("Tüm VIP'lere Süre Ekle", "DermaLarge", w/2, 22, Color(255, 193, 7), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        
        surface.SetDrawColor(255, 193, 7, 100)
        surface.DrawRect(0, 45, w, 2)
    end
    ...
    info:Dock(TOP)
    info:SetText("Bu işlem tüm aktif VIP'lerin süresine ekleme yapacaktır.")
    ...
    local vipCount = 0
    for _, line in pairs(self.VIPList:GetLines() or {}) do
        if line.data and line.data.expiry and tonumber(line.data.expiry) > 0 then
            vipCount = vipCount + 1
        end
    end
    
    local countLabel = vgui.Create("DLabel", content)
    countLabel:Dock(TOP)
    countLabel:SetText("Etkilenecek VIP sayısı: " .. vipCount)
    countLabel:SetFont("DermaDefaultBold")
    countLabel:SetTextColor(Color(255, 193, 7))
    countLabel:DockMargin(0, 0, 0, 20)
    ...
    daysEntry.Paint = function(s, w, h)
        ...
        s:DrawTextEntryText(Color(255, 255, 255), Color(255, 193, 7), Color(255, 255, 255))
    end
    ...
    -- Uyarı
    local warning = vgui.Create("DLabel", content)
    warning:Dock(TOP)
    warning:SetText("⚠ DİKKAT: Bu işlem geri alınamaz!")
    warning:SetFont("DermaDefaultBold")
    warning:SetTextColor(Color(255, 100, 100))
    warning:DockMargin(0, 10, 0, 20)
    
    -- Ekle butonu
    local addBtn = vgui.Create("DButton", content)
    addBtn:Dock(BOTTOM)
    addBtn:SetTall(45)
    addBtn:SetText("Tüm VIP'lere Süre Ekle")
    addBtn:SetFont("DermaDefaultBold")
    addBtn:SetTextColor(Color(255, 255, 255))
    addBtn.Paint = function(s, w, h)
        draw.RoundedBox(8, 0, 0, w, h, Color(255, 193, 7, s:IsHovered() and 255 or 200))
    end
    addBtn.DoClick = function()
        local days = tonumber(daysEntry:GetValue())
        if days and days > 0 then
            Derma_Query(
                "Tüm aktif VIP'lere " .. days .. " gün eklemek istediğinize emin misiniz?\n\nEtkilenecek VIP sayısı: " .. vipCount,
                "Onay Gerekli",
                "Evet, Ekle",
                function()
                    net.Start("SAM_VIP_Action")
                    net.WriteString("extendall")
...
net.Receive("SAM_VIP_SendList", function()
    ...
    for _, data in ipairs(vip_list) do
        ...
        if data.expiry and tonumber(data.expiry) and tonumber(data.expiry) > 0 then
            local expiry_num = tonumber(data.expiry)
            local time_left = expiry_num - os.time()
