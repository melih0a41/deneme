-- SAM VIP Yönetim Sistemi - Client Menü (TAMAMEN DÜZELTİLMİŞ)
...
function PANEL:OpenAddVIPMenu()
    ...
    -- HATA DÜZELTMESİ: Server tarafı ile uyumlu hale getirildi
    addbtn.DoClick = function()
        ...
        if steamid and typeid and time ~= nil then
            local vipType = typeid:gsub("vip$", "")
            
            -- DÜZELTİLDİ: Server tarafı "add" action'ını kullan
            net.Start("SAM_VIP_Action")
            net.WriteString("add")
            net.WriteString(steamid) -- # işareti OLMADAN
            net.WriteString(vipType)
            net.WriteUInt(time, 16) -- Dakika olarak gönder
            net.SendToServer()
