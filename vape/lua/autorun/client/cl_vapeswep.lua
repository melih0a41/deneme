...
function vape_do_particle(particle, vel, fx)
    particle:SetColor(200, 200, 200, 180)         -- hafif gri duman
	if fx == 3 then particle:SetColor(100, 255, 150, 200) end -- medicinal vape yeşil
	if fx == 31 then particle:SetColor(255, 60, 60, 200) end -- medicinal vape+ kırmızı
	if fx == 32 then particle:SetColor(60, 100, 255, 200) end -- armor vape+
	if fx == 34 then particle:SetColor(128, 0, 255) end -- armor medicine + mor
    particle:SetStartSize(1)                      -- küçük başlangıç
    particle:SetEndSize(4)                        -- hafif büyüsün
    particle:SetDieTime(math.Rand(0.1, 0.3))       -- 0.2 saniyede kaybolur
    particle:SetVelocity(vel * 0.6)               -- daha yavaş hareket eder
    particle:SetGravity(Vector(0, 0, 1))          -- hafif yukarı çıkar
    particle:SetAirResistance(100)                -- hızlıca yavaşlar
    particle:SetStartAlpha(180)                   -- başta yarı saydam
    particle:SetEndAlpha(0)                       -- yok olurken saydamlaşır
end
