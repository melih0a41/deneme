...
function SWEP:Think()
    ...
    -- CLIENT kontrolü ve rate limiting
    if CLIENT and self:GetOwner():KeyDown(IN_ATTACK) then
        -- NextVapeTick güvenlik kontrolü
        if CurTime() > (self.NextVapeTick or 0) then
            -- LocalPlayer kontrolü ve network spam koruması
