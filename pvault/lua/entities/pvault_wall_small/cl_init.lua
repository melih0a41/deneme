...
function ENT:Initialize()
	self.room = ClientsideModel("models/freeman/vault/pvault_wallsafe_smallinterior.mdl", RENDERGROUP_OPAQUE)
...
function ENT:Think()
	...
	self.room:SetPos(self:GetPos()+(self.room:GetForward()*1))
	self.room:SetAngles(self:GetAngles())
end
...
function ENT:Draw()
	...
	cam.Start3D2D(Pos + Ang:Up()*3 + Ang:Forward()*-15, Ang, 0.035)
	...
	local centerx, centery = -30, -755	

	cam.Start3D2D(Pos + Ang:Up()*6.5, Ang, 0.022)
