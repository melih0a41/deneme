...
function ENT:Initialize()
	self.room = ClientsideModel("models/freeman/vault/floor_safe_interior.mdl", RENDERGROUP_OPAQUE)
...
function ENT:Draw()
	...
	local centerx, centery = 150, -630


	cam.Start3D2D(Pos + Ang:Up()*-28 + Ang:Forward()*-35, Ang, 0.07)
	...
	local centerx, centery = 100, -90

	cam.Start3D2D(Pos+ Ang:Up()*22, Ang, 0.07)
		if self:GetCooldownEnd() > CurTime() then
			draw.RoundedBox(0, centerx-470, centery+50, 740, 100, Color(0, 0, 0, 200))
			draw.RoundedBox(0, centerx-470, centery+50, 740, 20, Color(0, 0, 0))
			draw.SimpleText(string.format(perfectVault.Translation.Vault.CooldownEnds, math.Round(self:GetCooldownEnd()- CurTime())), "_pvault70", 0, centery+110, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		elseif self:GetOpenTimeLeft() > CurTime() then
			draw.RoundedBox(0, centerx-470, centery+50, 740, 100, Color(0, 0, 0, 200))
			draw.RoundedBox(0, centerx-470, centery+50, 740, 20, Color(0, 0, 0))
			draw.SimpleText(string.format(perfectVault.Translation.Vault.VaultCloseTimer, math.Round(self:GetOpenTimeLeft()- CurTime())), "_pvault70", 0, centery+110, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		elseif self:GetAlarm() then
			draw.RoundedBox(0, centerx-470, centery+50, 740, 100, Color(0, 0, 0, 200))
			draw.RoundedBox(0, centerx-470, centery+50, 740, 20, Color(0, 0, 0))
			draw.SimpleText(perfectVault.Translation.Vault.VaultAlarmActive, "_pvault70", 0, centery+110, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		elseif player.GetCount() < tonumber(self.data.general.plyNeeded) then
			draw.RoundedBox(0, centerx-470, centery+50, 740, 100, Color(0, 0, 0, 200))
			draw.RoundedBox(0, centerx-470, centery+50, 740, 20, Color(0, 0, 0))
			draw.SimpleText(perfectVault.Translation.Vault.NoPlayers, "_pvault70", 0, centery+110, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	cam.End3D2D()
end
