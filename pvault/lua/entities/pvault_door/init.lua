...
function ENT:Initialize()
	self:SetModel("models/freeman/vault/vault_door.mdl")
...
function ENT:Unlock()
	...
	timer.Simple(5.7, function()
		if not IsValid(self) then return end
		local seq = self:LookupSequence("idleopen")
		self:ResetSequence(seq)
		self.doorUsable = true
	end)
...
function ENT:Lock()
	...
	timer.Simple(3.5, function()
		if not IsValid(self) then return end
		local seq = self:LookupSequence("idleclosed")
		self:ResetSequence(seq)
...
function ENT:Use(ply)
	...
	if timer.Exists("xyz_meeting_end_timer") and XYZMeeting.MeetingData.stopCrime then
		perfectVault.Core.Msg("There's an ongoing meeting...", ply) return
	end
