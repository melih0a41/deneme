...
function SWEP:PrimaryAttack()
    ...
    _G.KidnapData[target] = {
        ragdoll = ragdoll,
        timer = CurTime() + 30, -- Bu CurTime() kalabilir çünkü kısa süreli (30 saniye PRO versiyon)
        kidnapper = ply,
...
--[[---------------------------------------------------------
    HOOKS - Sunucu Tarafı Kontroller (sadece bir kez tanımla)
-----------------------------------------------------------]]
if SERVER and not _G.KidnapHooksLoaded then
    _G.KidnapHooksLoaded = true

    -- Kurbanın serbest bırakılmasını sürekli kontrol et
    hook.Add("Think", "KidnapperThinkPro", function()
        local nextCheck = _G.KidnapperNextCheckPro or 0
        if CurTime() < nextCheck then return end
        _G.KidnapperNextCheckPro = CurTime() + 0.5
