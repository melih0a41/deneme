...
function SWEP:PrimaryAttack()
    ...
    if IsValid(target) and target:IsPlayer() and target:Alive() and ply:GetPos():DistToSqr(target:GetPos()) < 10000 then
        ...
        -- Kurbanın silahlarını al (Artık kaydedildi)
        target:StripWeapons()
        ...
        _G.KidnapData[target] = {
            ragdoll = ragdoll,
            timer = CurTime() + 15, -- Serbest bırakma zamanlayıcısı (saniye)
            kidnapper = ply,
...
function SWEP:Think()
    ...
    if ply:KeyDown(IN_ATTACK2) then
        if not self.IsDragging then
            ...
            -- ply:ChatPrint(self.Victim:Nick() .. "'nin bedenini sürükleniyor.") -- İsteğe bağlı
        end
        ...
        if self.IsDragging then
            ...
            -- ply:ChatPrint(self.Victim:Nick() .. "'nin bedenini sürükleme durduruldu.") -- İsteğe bağlı
        end
    end
end
