...
SWEP.ClassName = "weapon_kidnapuser"
SWEP.PrintName = "Kaçırma Aleti"
...
-- Server tarafında cooldown sistemi
if SERVER then
    ...
    -- Dosya yolu
    local dataPath = "kidnap_cooldowns.txt"
    
    -- Veriyi kaydet
    function SaveKidnapCooldowns()
        local dataToSave = {
    ...
    -- Veriyi yükle
    local function LoadKidnapCooldowns()
    ...
    hook.Add("Initialize", "LoadKidnapCooldowns", function()
        timer.Simple(1, LoadKidnapCooldowns)
    end)
    
    -- Oyuncu ayrıldığında veriyi kaydet
    hook.Add("PlayerDisconnected", "SaveKidnapCooldownOnLeave", function(ply)
        SaveKidnapCooldowns()
    end)
    
    -- Sunucu kapandığında veriyi kaydet
    hook.Add("ShutDown", "SaveKidnapCooldownsOnShutdown", SaveKidnapCooldowns)
    
    -- Her 5 dakikada bir otomatik kaydet
    timer.Create("AutoSaveKidnapCooldowns", 300, 0, SaveKidnapCooldowns)
    ...
    local function CanPlayerKidnap(ply)
    ...
    -- Meslek kontrolü
    local function CanTeamKidnap(ply)
    ...
    local function SetKidnapCooldown(ply)
        local steamID = ply:SteamID()
    ...
    hook.Add("PlayerSpawn", "SendKidnapCooldown", function(ply)
    ...
    hook.Add("PlayerInitialSpawn", "CheckKidnapCooldownOnJoin", function(ply)
...
function SWEP:PrimaryAttack()
    ...
    -- Debug için mesaj
    if isRetryingSameTarget then
        ply:ChatPrint("DEBUG: Aynı hedef tespit edildi, cooldown atlanıyor.")
    else
        ply:ChatPrint("DEBUG: Farklı hedef, cooldown kontrol ediliyor.")
    end
    
    -- Eğer hiç kaçırmadı veya farklı birini kaçırmaya çalışıyorsa cooldown kontrol et
    if not isRetryingSameTarget then
        local canKidnap, remainingTime = CanPlayerKidnap(ply)
    ...
    _G.KidnapData[target] = {
        ragdoll = ragdoll,
        timer = CurTime() + 15, -- Bu CurTime() kalabilir çünkü kısa süreli
        kidnapper = ply,
...
if SERVER then
    ...
    -- Kurbanın serbest bırakılmasını sürekli kontrol et
    ...
    -- Debug komutları
    concommand.Add("kidnap_cooldown_list", function(ply, cmd, args)
        if IsValid(ply) and not ply:IsSuperAdmin() then return end
        
        print("=== Active Kidnap Cooldowns ===")
        local currentTime = os.time()
    ...
    concommand.Add("kidnap_cooldown_clear", function(ply, cmd, args)
        if IsValid(ply) and not ply:IsSuperAdmin() then return end
        ...
        if args[1] then
            ...
            if _G.KidnapCooldowns[targetSteamID] then
                ...
                SaveKidnapCooldowns()
                print("Cleared cooldown for: " .. targetSteamID)
            else
                print("No cooldown found for: " .. targetSteamID)
            end
        else
            _G.KidnapCooldowns = {}
            _G.KidnapLastTargets = {}
            SaveKidnapCooldowns()
            print("All kidnap cooldowns cleared!")
        end
    end)
...
if CLIENT then
    ...
    local function DrawKidnapCooldownPanel()
        ...
        -- Başlık
        draw.SimpleText("KIDNAP COOLDOWN", "DermaDefaultBold", panelX + panelW/2, panelY + 15, Color(255, 255, 255, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        ...
        -- Progress bar dolgu
        local totalCooldown = 15 * 60 -- 15 dakika
        local progress = 1 - (remainingTime / totalCooldown)
    ...
    if not _G.KidnapInitHookLoaded then
        ...
        hook.Add("Initialize", "KidnapCooldownClientInitMain", function()
            timer.Simple(3, function()
                chat.AddText(Color(255, 165, 0), "[Kidnap Sistemi] ", Color(255, 255, 255), "Kidnap cooldown sistemi aktif!")
            end)
        end)
    end
end