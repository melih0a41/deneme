...
function ENT:Initialize()
	...
	self.Rope = constraint.Rope( self, self.PowerPlug, 0, 0, self:WorldToLocal( self:GetAttachment( 1 ).Pos ), self.PowerPlug:WorldToLocal( self.PowerPlug:GetAttachment( 1 ).Pos ), CH_Bitminers.Config.CableRopeLenght, 50, 0, 1, "cable/cable2", false, CH_Bitminers.Config.CableRopeColor )
...
function ENT:StartTouch( ent )
	...
	ent.LastTouch = cur_time + 0.5
	
	local owner = self:CPPIGetOwner()
	
	if ent:GetClass() == "ch_bitminer_shelf" then
		if ent:GetHasPower() then
			CH_Bitminers.NotifyPlayer( owner, CH_Bitminers.LangString( "The bitminer already has a power source connected!" ) )
			return
		end
		if not self.HasPowerSource then
			CH_Bitminers.NotifyPlayer( owner, CH_Bitminers.LangString( "Please attach the cable to a power source first!" ) )
			return
		end
		...
		if ent.ConnectedCables < 5 then
			...
			if not ent.RopeToShelf then
				if self.Parent.HasPowerSource then
					CH_Bitminers.NotifyPlayer( owner, CH_Bitminers.LangString( "Please attach the power combiner to a shelf first!" ) )
					return
				end
