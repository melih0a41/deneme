...
XeninInventory.Config.PrefixText = "[Envanter Sistemi]"
...
XeninInventory.Config.NPCText = "Envanter Bankasi"
...
XeninInventory.Config.Categories = {
  [1] = { name = "Yaygın", color = Color(125, 125, 125), maxStack = 50 }, -- grey
  [2] = { name = "Az Yaygın", color = Color(46, 204, 113), maxStack = 30 }, -- green
...
XeninInventory.Config.HolsterTime = {
    ...
    ["arccw_bo1_ak47"] = 15,
    ["arccw_bo1_ak74u"] = 15,
    ["arccw_bo2_ballista"] = 15,
    ["arccw_bo1_dragunov"] = 15,
    ["arccw_waw_k98k"] = 15,
    ["arccw_bo2_sig556"] = 15,
    ["arccw_bo2_thompson"] = 15,
    ["tfa_cso_scout"] = 15,
    ["tfa_cso_trg42"] = 15,
}
...
local funcHolster = function(ply)
  ...
  -- Eylem bayrağını kontrol et
  if (ply.isInventoryActionBusy) then
    ply:XeninInventory():Message("Zaten bir eylem yapıyorsun, lütfen bekle.")
    return
  end
  ...
  -- Eylemi başlatmadan önce bayrağı ayarla
  ply.isInventoryActionBusy = true

  ply:dropDRPWeapon(activeWep, function(wep)
    ply:XeninInventory():Pickup(wep)
    -- Eylem tamamlandığında bayrağı temizle
    ply.isInventoryActionBusy = false
  end, true)
end
...
["/drop"] = function(ply)
    ...
    -- /drop işlemi anlık olduğu için bayrağı kısa bir süre sonra temizleyebiliriz.
