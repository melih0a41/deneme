...
-- Oyuncunun gerçek mesleğini almak için yardımcı fonksiyon
local function GetPlayerJobName(ply)
    if not IsValid(ply) then return "Unknown" end
    
    -- Özel meslek adını kontrol et
    local customJob = ply:getDarkRPVar('customJob')
    
    if customJob and customJob ~= "" then
        -- Özel meslek varsa onu döndür
        return customJob
    else
        -- Yoksa normal mesleği döndür
        return ply:getDarkRPVar('job') or team.GetName(ply:Team())
    end
end
...
do
    function PANEL:FetchCategory(ply)
        if (DarkRP and onyx.scoreboard:GetOptionValue('group_teams')) then
            ...
            if (jobData) then
                -- Özel meslek kontrolü
                local customJob = ply:getDarkRPVar('customJob')
                if customJob and customJob ~= "" then
                    -- Özel meslek varsa kategori olarak kullan
                    return customJob, team.GetColor(ply:Team())
                else
                    -- Normal kategori
        ...
        -- DarkRP için özel meslek kontrolü
        if DarkRP then
            return GetPlayerJobName(ply), team.GetColor(ply:Team())
        end
