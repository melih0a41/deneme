...
local PartyFrame
local OpenToRaidTab = false  -- Raid sekmesine direkt açılması için flag
		...
		hook.Add( "Think", "Partykeylistener", function()
			...
			-- Raid menüsü açma network receiver
...
function PartyMenu()
	-- Eğer başka bir menü açıksa kapat
	...
	-- Z-Index ayarı - diğer panellerin üstünde görünsün
	PartyFrame:SetZPos(100)
	
	-- Panel kapandığında global değişkeni temizle
	PartyFrame.OnClose = function()
		PartyFrame = nil
		OpenToRaidTab = false -- Reset flag
	end
...
local Partylist = vgui.Create("PartyCategories", PartyFrame)
	...
	-- RAID MENÜSÜ SEKMESİ
	-- ============================
	Partylist:NewItem("Raid Menüsü", Color(255, 100, 0), function()
		if Partytab then
			Partytab:Remove()
		end
		PartyFrame:SetSize(party.framesizex, party.framesizey)
		...
		if not partyID then
			-- Parti yoksa uyarı
			local DLabel = vgui.Create("PartyLabel", Partytab)
			DLabel:SetColor(Color(200, 0, 0))
			DLabel:SetText("RAID BAŞLATMAK İÇİN")
			DLabel:SizeToContents()
			DLabel:Center()
			DLabel:AlignTop(60)
			
			local DLabel2 = vgui.Create("PartyLabel", Partytab)
			DLabel2:SetColor(Color(200, 0, 0))
			DLabel2:SetText("PARTİ KURMANIZ GEREKİYOR!")
		...
		if partyID != LocalPlayer():SteamID64() then
			-- Lider değilse uyarı
			local DLabel = vgui.Create("PartyLabel", Partytab)
			DLabel:SetColor(Color(200, 0, 0))
			DLabel:SetText("SADECE PARTİ LİDERİ")
			DLabel:SizeToContents()
			DLabel:Center()
			DLabel:AlignTop(60)
			...
			local DLabel2 = vgui.Create("PartyLabel", Partytab)
			DLabel2:SetColor(Color(200, 0, 0))
			DLabel2:SetText("RAID BAŞLATABİLİR!")
			DLabel2:SizeToContents()
			DLabel2:Center()
			DLabel2:AlignTop(80)
		...
		-- Başlık
		local titleLabel = vgui.Create("PartyLabel", Partytab)
		titleLabel:SetColor(Color(255, 100, 0))
		titleLabel:SetText("RAID HEDEF SEÇ")
		titleLabel:SizeToContents()
		titleLabel:Center()
		titleLabel:AlignTop(10)
		
		-- Parti listesi için scroll panel
		local scrollPanel = vgui.Create("DScrollPanel", Partytab)
		scrollPanel:SetPos(5, 35)
		scrollPanel:SetSize(party.rightsize - 10, 180)
		
		-- Seçili parti için değişken
		local selectedPartyID = nil
		local selectedButton = nil
		
		-- Partileri listele
		local yPos = 0
		for targetPartyID, partyData in pairs(parties) do
			if targetPartyID != partyID then
				-- Parti butonu
				local partyBtn = vgui.Create("DButton", scrollPanel)
				partyBtn:SetPos(0, yPos)
				partyBtn:SetSize(scrollPanel:GetWide() - 15, 30)
				partyBtn:SetText("")
				
				partyBtn.Paint = function(self, w, h)
					local color = Color(60, 60, 60, 200)
					
					if self == selectedButton then
						color = Color(100, 150, 0, 200)
					...
					-- Parti adı
					draw.SimpleText(partyData.name, "roboto16", 5, h/2, Color(255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
					...
					local statusText = alive .. "/" .. online .. " Aktif"
					local statusColor = alive >= 1 and Color(0, 255, 0) or Color(255, 100, 100)
					draw.SimpleText(statusText, "roboto16", w - 5, h/2, statusColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
				end
				
				partyBtn.DoClick = function()
					selectedPartyID = targetPartyID
					selectedButton = partyBtn
				end
		...
		-- Raid başlat butonu
		local raidBtn = vgui.Create("PartyButton", Partytab)
		raidBtn:SetText("RAID BAŞLAT")
		raidBtn:SetSize(party.rightsize, 40)
		raidBtn:SetTextColor(Color(255, 255, 255))
		raidBtn:Dock(BOTTOM)
		raidBtn:DockMargin(0, 10, 0, 5)
		
		raidBtn.DoClick = function()
			if not selectedPartyID then
				notification.AddLegacy("Hedef parti seçmelisiniz!", NOTIFY_ERROR, 3)
				return
			end
			
			net.Start("PropHP_RaidRequest")
				net.WriteString(selectedPartyID)
			net.SendToServer()
			PartyFrame:Close()
		end
		
		-- Aktif raid varsa iptal butonu göster
		if PropHP_Client and PropHP_Client.RaidData and PropHP_Client.LocalPlayerSide == "attacker" then
			local cancelBtn = vgui.Create("PartyButton", Partytab)
			cancelBtn:SetText("RAID İPTAL ET")
			cancelBtn:SetSize(party.rightsize, 40)
			cancelBtn:SetTextColor(Color(255, 255, 255))
			cancelBtn:Dock(BOTTOM)
			cancelBtn:DockMargin(0, 0, 0, 0)
			
			cancelBtn.Paint = function(self, w, h)
				local color = self:IsHovered() and Color(200, 50, 50) or Color(150, 0, 0)
			...
			cancelBtn.DoClick = function()
				Derma_Query(
					"Raid'i iptal etmek istediğinizden emin misiniz?",
					"Raid İptal Onay",
					"Evet",
					function()
	...
	-- Eğer raid sekmesine direkt açılması isteniyorsa
	if OpenToRaidTab then
		OpenToRaidTab = false
		-- Raid sekmesini simüle et
		for _, item in pairs(Partylist.Items) do
			if item.text and item.text:GetText() == "Raid Menüsü" then
				item:OnMousePressed()
				break
			end
		end
