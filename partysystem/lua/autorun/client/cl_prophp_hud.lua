...
function PropHP_DrawPropInfo()
    ...
    -- Panel
    local w = 150
    local h = 75  -- Yüksekliği arttırdım E tuşu bilgisi için
    ...
    if targetParty != "" and myParty and myParty == ply:SteamID64() and targetParty != myParty then
        ...
        if canShowRaidPrompt then
            -- E tuşu bilgisi göster
            local promptY = y + h + 5
            draw.RoundedBox(4, x, promptY, w, 20, Color(255, 100, 0, 200))
            draw.SimpleText("[E] RAID BAŞLAT", "PropHP_Small", screenPos.x, promptY + 10, Color(255, 255, 255), TEXT_ALIGN_CENTER)
        end
    end
end
