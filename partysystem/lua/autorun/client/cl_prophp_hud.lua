...
-- Versiyon: 2.4 - Raid menüsü parti menüsüne taşındı
...
hook.Add("PreDrawHalos", "PropHP_RaidGlow", function()
    ...
    -- HAZIRLIK AŞAMASINDA GLOW GÖSTERME - YENİ!
    if PropHP_Client.RaidData and PropHP_Client.RaidData.preparation then
        return -- Hazırlık aşamasında glow yok
    end
...
function PropHP_DrawPropInfo()
    ...
    -- MESAFE KONTROLÜ - Config'den mesafeyi al (varsayılan 250)
    ...
    local myParty = ply:GetParty()
    
    -- Kontroller: Parti lideri mi ve başka partiye mi ait prop?
    if targetParty != "" and myParty and myParty == ply:SteamID64() and targetParty != myParty then
        ...
        if canShowRaidPrompt then
            -- E tuşu bilgisi göster - TURUNCU KUTU İÇİNDE
            local promptY = y + h + 5
            local promptH = 25  -- Kutu yüksekliği
            
            -- Turuncu arka plan kutusu
            draw.RoundedBox(4, x, promptY, w, promptH, Color(255, 100, 0, 200))
            
            -- [E] RAID BAŞLAT yazısı - kutunun tam ortasında
