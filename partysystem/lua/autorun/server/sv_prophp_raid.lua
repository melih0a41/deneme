-- Raid System - Server Side (DÜZELTILMIŞ VERSIYON)
-- Dosya Yolu: lua/autorun/server/sv_prophp_raid.lua
-- NLR, 5v5 Scoreboard ve Parti Ölüm Kontrolü Dahil
...
-- ============================
-- RAID KATILIMCI GÜNCELLEMESİ (5v5 SCOREBOARD İÇİN)
-- ============================
...
-- ============================
-- ÖDÜL SİSTEMİ (KALDIRILDI - SADECE BİLDİRİM)
-- ============================
    ...
    for raidID, raid in pairs(PropHP.ActiveRaids) do
        ...
        if participants and (raid.active or raid.lootingPhase) then
            ...
            if participants.attackers[steamID] and not participants.attackers[steamID].alive then
                ...
                -- Sadece izleyebilir, hasar veremez
            end
...
-- ============================
-- NLR HASAR ENGELLEMESİ
-- ============================
hook.Add("EntityTakeDamage", "PropHP_NLRDamageBlock", function(target, dmginfo)
    ...
    -- Raid'de NLR'li mi kontrol et
    for raidID, raid in pairs(PropHP.ActiveRaids) do
        local participants = PropHP.RaidParticipants[raidID]
        if participants and (raid.active or raid.lootingPhase) then
            -- NLR'li oyuncu hasar veremez
    ...
    elseif args[1] == "!prophp" then
        ...
        if data.raidStatus then
            if data.raidStatus.preparation then
                ply:ChatPrint("⏰ Raid hazırlık aşamasında!")
            elseif data.raidStatus.active then
                ply:ChatPrint("⚔️ Raid aktif!")
            elseif data.raidStatus.lootingPhase then
                ply:ChatPrint("💰 Yağma aşamasında!")
            end
        end
