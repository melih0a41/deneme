...
util.AddNetworkString("PropHP_CancelRaid")  -- YENİ
...
function PropHP.RaidSystem.StartRaid(attackerParty, defenderParty)
    ...
    for _, ply in pairs(player.GetAll()) do
        ply:ChatPrint("⚔️ ===============================")
        ply:ChatPrint("⚔️ RAID BAŞLADI! (Hazırlık Aşaması)")
        ply:ChatPrint("⚔️ Saldıran: " .. attackerName)
        ply:ChatPrint("🛡️ Savunan: " .. defenderName) 
        ply:ChatPrint("⏰ " .. PropHP.Config.Raid.PreparationTime/60 .. " dakika hazırlık süresi")
        ply:ChatPrint("⚔️ ===============================")
    end
...
function PropHP.RaidSystem.CancelRaid(ply)
    ...
    for _, p in pairs(player.GetAll()) do
        p:ChatPrint("⚠️ ===============================")
        p:ChatPrint("⚠️ RAID İPTAL EDİLDİ!")
        p:ChatPrint("⚠️ " .. (parties[raid.attackerParty] and parties[raid.attackerParty].name or "???") .. " partisi raid'i iptal etti!")
        p:ChatPrint("⚠️ ===============================")
    end
    ...
    net.Start("PropHP_RaidEnd")
        net.WriteString("canceled")  -- İptal edildi
        net.WriteString("")
        net.WriteString("")
    net.Broadcast()
...
function PropHP.RaidSystem.ActivateRaid(raidID)
    ...
    -- TÜM OYUNCULARA RAID AKTİF BİLDİRİMİ - YENİ
    ...
    for _, ply in pairs(player.GetAll()) do
        -- Raid katılımcısı değilse genel bildirim
        local isParticipant = false
        if parties[raid.attackerParty] and table.HasValue(parties[raid.attackerParty].members, ply:SteamID64()) then
            isParticipant = true
        end
        if parties[raid.defenderParty] and table.HasValue(parties[raid.defenderParty].members, ply:SteamID64()) then
            isParticipant = true
        end
        
        if not isParticipant then
            ply:ChatPrint("⚔️ ===============================")
            ply:ChatPrint("⚔️ HAZIRLIK BİTTİ - SAVAŞ BAŞLADI!")
            ply:ChatPrint("⚔️ " .. attackerName .. " VS " .. defenderName)
            ply:ChatPrint("⚔️ ===============================")
        end
    end
    ...
    if parties[attackerParty] then
        for _, steamID in pairs(parties[attackerParty].members) do
            ...
            if IsValid(member) then
                ...
                if preparation then
                    member:ChatPrint("⏰ ===============================")
                    member:ChatPrint("⏰ RAID HAZIRLANIYOR!")
                    member:ChatPrint("⏰ " .. PropHP.Config.Raid.PreparationTime/60 .. " dakika hazırlık süresi!")
    ...
    if parties[defenderParty] then
        for _, steamID in pairs(parties[defenderParty].members) do
            ...
            if IsValid(member) then
                ...
                if preparation then
                    member:ChatPrint("⚠️ ===============================")
                    member:ChatPrint("⚠️ RAID UYARISI!")
                    member:ChatPrint("⚠️ " .. PropHP.Config.Raid.PreparationTime/60 .. " dakika hazırlanın!")
    ...
    if winner == "attacker" and not raid.lootingPhase then
        ...
        if parties[raid.defenderParty] then
            for _, steamID in pairs(parties[raid.defenderParty].members) do
                ...
                if IsValid(member) then
                    member:ChatPrint("⏰ ===============================")
                    member:ChatPrint("⏰ Saldıranlar yağmalıyor!")
                    member:ChatPrint("⏰ 3 dakika sonra prop'lar tamir olacak.")
                    member:ChatPrint("⚠️ Yağma sırasında da prop koyamazsınız!")
                    member:ChatPrint("⏰ ===============================")
                    table.insert(allMembers, member)
                end
...
function PropHP.RaidSystem.FinalizeRaid(raidID, overrideWinner)
    ...
    for _, ply in pairs(player.GetAll()) do
        ply:ChatPrint("🏆 ===============================")
        ply:ChatPrint("🏆 RAID SONA ERDİ!")
        ply:ChatPrint("🏆 Kazanan: " .. winnerName)
        ply:ChatPrint("💀 Kaybeden: " .. loserName)
        ply:ChatPrint("🏆 ===============================")
    end
...
hook.Add("PlayerSay", "PropHP_ChatCommands", function(ply, text)
    ...
    elseif args[1] == "!raidiptal" then  -- YENİ KOMUT
...
-- RAID İPTAL NETWORK RECEIVER - YENİ
net.Receive("PropHP_CancelRaid", function(len, ply)
    PropHP.RaidSystem.CancelRaid(ply)
end)

-- Sistem yüklendi bildirimi
print("[PropHP] Raid sistemi yüklendi - v2.2 CANCEL FEATURE")