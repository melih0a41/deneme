...
-- Versiyon: 3.0 - Tüm güvenlik ve performans sorunları düzeltildi
...
PropHP.RaidTimers = PropHP.RaidTimers or {} -- Timer takibi için
...
util.AddNetworkString("PropHP_PropRaidPrompt")

-- ============================
-- RAID VALİDASYON VE GÜVENLİK
-- ============================
function PropHP.RaidSystem.ValidateRaidRequest(ply, targetParty)
    -- Temel validasyonlar
    if not IsValid(ply) then return false, "Geçersiz oyuncu!" end
    
    -- String validasyon
    if type(targetParty) != "string" or targetParty == "" then
        return false, "Geçersiz hedef parti!"
    end
    
    -- SQL injection koruması (basit)
    if string.find(targetParty, "[';\"\\]") then
        return false, "Geçersiz karakter tespit edildi!"
    end
    
    -- Rate limiting
    local steamID = ply:SteamID64()
    PropHP.RaidSystem.RateLimits = PropHP.RaidSystem.RateLimits or {}
    
    if PropHP.RaidSystem.RateLimits[steamID] then
        if CurTime() - PropHP.RaidSystem.RateLimits[steamID] < 5 then
            return false, "Çok hızlı istek gönderiyorsunuz!"
        end
    end
    
    PropHP.RaidSystem.RateLimits[steamID] = CurTime()
    
    return true, "OK"
end

-- ============================
-- TIMER TEMİZLEME FONKSİYONU
-- ============================
function PropHP.RaidSystem.CleanupTimers(raidID)
    if not raidID then return end
    
    -- Bu raid için tüm timer'ları temizle
    local timers = {
        "PropHP_PrepTimer_" .. raidID,
        "PropHP_UpdateTimer_" .. raidID,
        "PropHP_EndTimer_" .. raidID,
        "PropHP_LootTimer_" .. raidID
    }
    ...
    -- Timer listesinden kaldır
    PropHP.RaidTimers[raidID] = nil
    
    -- Log
    if PropHP.Config.Debug.Enabled then
        print("[PropHP] Raid timer'ları temizlendi: " .. raidID)
    end
end
...
function PropHP.RaidSystem.StartRaid(attackerParty, defenderParty)
    ...
    -- Timer listesine ekle
    PropHP.RaidTimers[raidID] = true
    ...
    -- Log
    ServerLog("[PropHP] Raid başlatıldı - " .. attackerName .. " vs " .. defenderName .. "\n")
end
...
function PropHP.RaidSystem.CancelRaid(ply)
    ...
    -- TÜM PROP'LARI TAMİR ET VE FULL HP YAP
    PropHP.RestoreAllProps(raid.defenderParty)
    PropHP.RestoreAllProps(raid.attackerParty)
    ...
    -- Timer'ları temizle
    PropHP.RaidSystem.CleanupTimers(raidID)
...
-- ============================
-- TÜM PROP'LARI RESTORE ET
-- ============================
function PropHP.RestoreAllProps(partyID)
    ...
    -- Ghost propları tamir et
    for _, prop in ipairs(data.ghostProps or {}) do
    ...
    -- Tüm propların HP'sini full yap
    for _, prop in pairs(data.props) do
        if IsValid(prop) then
            local maxHP = prop:GetNWInt("PropMaxHP", 0)
            if maxHP > 0 then
                local currentHP = prop:GetNWInt("PropHP", 0)
                if currentHP < maxHP then
                    prop:SetNWInt("PropHP", maxHP)
    ...
    -- Yok edilmiş prop sayısını sıfırla
    ...
    if parties and parties[partyID] and parties[partyID].members then
        for _, steamID in pairs(parties[partyID].members) do
            ...
            if IsValid(ply) then
                local message = "Raid iptal edildi! "
                if repairedCount > 0 then
                    message = message .. repairedCount .. " ghost prop tamir edildi. "
                end
                if restoredCount > 0 then
                    message = message .. restoredCount .. " prop full HP'ye döndü."
                end
    ...
    return repairedCount, restoredCount
end

-- ============================
-- RAID KATILIMCI GÜNCELLEMESİ - OPTİMİZE
-- ============================
function PropHP.RaidSystem.UpdateParticipants(raidID)
    ...
    -- Cache kontrolü
    participants.lastUpdate = participants.lastUpdate or 0
    if CurTime() - participants.lastUpdate < 1 then
...
function PropHP.RaidSystem.UpdateRaidTimer(raidID)
    ...
    if participants and raid.active and not raid.lootingPhase then
        ...
        for steamID, data in pairs(participants.attackers) do
            if data.alive and data.initialAlive then
                attackerAlive = attackerAlive + 1
            end
        end
...
function PropHP.RaidSystem.FinalizeRaid(raidID, overrideWinner)
    ...
    -- Log
    ServerLog("[PropHP] Raid sona erdi - Kazanan: " .. winnerName .. "\n")
end
...
hook.Add("PlayerSay", "PropHP_ChatCommands", function(ply, text)
    ...
    elseif args[1] == "!prophp" then
        ...
        local ghostProps = #(data.ghostProps or {})
        
        ply:ChatPrint("HP Havuzu: " .. string.Comma(data.totalPool))
        ply:ChatPrint("Toplam Prop: " .. (aliveProps + ghostProps) .. " (Sağlam: " .. aliveProps .. ", Ghost: " .. ghostProps .. ")")
        
        if aliveProps > 0 then
            ply:ChatPrint("HP/Prop: " .. string.Comma(math.floor(data.totalPool / aliveProps)))
        end
    ...
    elseif args[1] == "!raidstatus" and ply:IsSuperAdmin() then
        ...
        ply:ChatPrint("=== AKTİF TIMER'LAR ===")
        for raidID, _ in pairs(PropHP.RaidTimers) do
            ply:ChatPrint("  " .. raidID)
        end
...
-- ============================
-- NETWORK RECEIVERS - GÜVENLİK İYİLEŞTİRMELERİ
-- ============================
net.Receive("PropHP_RaidRequest", function(len, ply)
    if not IsValid(ply) then return end
    
    -- Rate limiting
    if not PropHP.CanSendNetwork(ply, "raid_request") then
        ply:ChatPrint("Çok hızlı istek gönderiyorsunuz!")
        return
    end
    ...
    -- Validasyon
    local valid, reason = PropHP.RaidSystem.ValidateRaidRequest(ply, targetParty)
    if not valid then
        ply:ChatPrint(reason)
        return
    end
    ...
    local canRaid, raidReason = PropHP.RaidSystem.CanStartRaid(attackerParty, targetParty)
    
    if canRaid then
        PropHP.RaidSystem.StartRaid(attackerParty, targetParty)
        ply:ChatPrint("Raid başlatıldı!")
    else
        ply:ChatPrint(raidReason)
    end
end)
...
net.Receive("PropHP_CancelRaid", function(len, ply)
    ...
    -- Rate limiting
    if not PropHP.CanSendNetwork(ply, "raid_cancel") then
        ply:ChatPrint("Çok hızlı istek gönderiyorsunuz!")
        return
    end
...
-- ============================
hook.Add("ShutDown", "PropHP_CleanupRaids", function()
    -- Tüm timer'ları temizle
    for raidID, _ in pairs(PropHP.RaidTimers) do
        PropHP.RaidSystem.CleanupTimers(raidID)
    end
    
    -- Tüm raid verilerini temizle
    PropHP.ActiveRaids = {}
    PropHP.RaidParticipants = {}
    PropHP.RaidCooldowns = {}
end)

-- Sistem yüklendi bildirimi
print("[PropHP] Raid sistemi yüklendi - v3.0 Tüm güvenlik ve performans sorunları düzeltildi")