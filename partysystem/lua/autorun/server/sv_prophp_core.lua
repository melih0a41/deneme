...
-- ============================
-- PROP HASAR SİSTEMİ (RAID KONTROLÜ EKLENDİ)
-- ============================
...
-- ============================
-- OYUNCU HASAR KONTROLÜ (RAID'DE SADECE RAID OYUNCULARI)
-- ============================
hook.Add("PlayerShouldTakeDamage", "PropHP_RaidPlayerDamage", function(victim, attacker)
    ...
    -- Raid kontrolü
    local victimData = PropHP.GetPartyData(victimParty)
    local attackerData = PropHP.GetPartyData(attackerParty)
    
    -- İkisinden biri raid'de mi?
    if (victimData and victimData.raidStatus) or (attackerData and attackerData.raidStatus) then
        local raidID = nil
        
        -- Raid ID'yi bul
        if victimData and victimData.raidStatus then
            raidID = victimData.raidStatus.raidID
        elseif attackerData and attackerData.raidStatus then
            raidID = attackerData.raidStatus.raidID
        end
        
        if raidID then
            -- Her iki oyuncu da bu raid'de mi?
            local victimInRaid = PropHP.IsPlayerInRaid(victim, raidID)
            local attackerInRaid = PropHP.IsPlayerInRaid(attacker, raidID)
            
            -- İkisi de raid'de değilse hasar veremez
            if not (victimInRaid and attackerInRaid) then
                return false
            end
            
            -- İkisi de raid'de ama aynı taraftaysa hasar veremez
            local victimSide = PropHP.GetPlayerRaidSide(victim, raidID)
            local attackerSide = PropHP.GetPlayerRaidSide(attacker, raidID)
            ...
            if participants then
                ...
                -- Saldıran ölü mü?
                if (participants.attackers[attackerSteamID] and not participants.attackers[attackerSteamID].alive) or
                   (participants.defenders[attackerSteamID] and not participants.defenders[attackerSteamID].alive) then
                    return false
                end
            end
        end
...
print("[PropHP] Core sistem yüklendi - v2.0")