...
hook.Add("KeyPress", "dex_RagdollDragSystem", function(ply, key)
    ...
    if trace.Hit and trace.Entity == ent then
        ...
        playerDragData[ply] = {
            Entity = ent,
            OffPos = ent:WorldToLocal(trace.HitPos),
...
hook.Add("Think", "dex_RagdollDragThink", function()
    for ply, dragData in pairs(playerDragData) do
        if IsValid(ply) and IsValid(dragData.Entity) and ply:KeyDown(IN_ATTACK) then
