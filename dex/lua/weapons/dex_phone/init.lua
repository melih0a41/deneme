...
local function RegisterPurchasedEntity(ply, entity, jobTeam)
    ...
    table.insert(PlayerJobEntities[steamID][teamID], entity)
    
    for i = #PlayerJobEntities[steamID][teamID], 1, -1 do
        if not IsValid(PlayerJobEntities[steamID][teamID][i]) then
            table.remove(PlayerJobEntities[steamID][teamID], i)
        end
    end
end
...
local function RemoveJobEntities(ply, jobTeam)
    ...
    for i, entity in ipairs(PlayerJobEntities[steamID][jobTeam]) do
...
function SWEP:DoPurchase(itemIndex)
    ...
    if item.isSWEP then
        if not ply:HasWeapon(item.entidade) then
            ...
            net.Start("Dex_BuySWEP_Notify")
                net.WriteUInt(1, 8)
                net.WriteString(DEX_LANG.Get("already_have"))
            net.Send(ply)
            return
        end
    else
        ...
        RegisterPurchasedEntity(ply, ent, ply:Team())
...
hook.Add("PlayerDisconnected", "Dex_CleanupPlayerData", function(ply)
    ...
    if PlayerJobEntities[steamID] then
        for jobTeam, entities in pairs(PlayerJobEntities[steamID]) do
