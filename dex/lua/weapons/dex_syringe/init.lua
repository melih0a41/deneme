...
function SWEP:PrimaryAttack()
    ...
    -- COOLDOWN KONTROLÜ
    local onCooldown, timeLeft = IsOnCooldown(owner)
    if onCooldown then
        net.Start("dex_CooldownWarning")
            net.WriteString(string.format("Tekrar avlanmak için %.0f saniye beklemelisin!", timeLeft))
        net.Send(owner)
    ...
    if IsValid(target) and (target:IsPlayer() or target:IsNPC()) and trace.HitPos:Distance(owner:GetShootPos()) <= 100 then
        ...
        if target:IsPlayer() then
            ...
            -- COOLDOWN'U BAŞLAT (Kaçırma başarılı olduğunda)
            StartKillCooldown(owner)
            
            -- Bildirim gönder
            if DarkRP then
                DarkRP.notify(owner, 1, 5, "Dikkatli ol! Tekrar avlanmak için 10 dakika beklemelisin.")
            else
                owner:ChatPrint("Dikkatli ol! Tekrar avlanmak için 10 dakika beklemelisin.")
            end
